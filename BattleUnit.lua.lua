...
function BattleUnit.NewUnit(data)
  ...
  function unit:InitBaseData(transformReset, keepSkillIndex)
    if true ~= transformReset then
      ...
      self.curValue_1052 = 0
      self.maxValue_1052 = 1000
    end
    ...
    self.useSummonModelPosition = nil
  ...
  function unit:InitData(dataParam)
    ...
    if self.bindUnitSlot then
      self.useSummonModelPosition = true
    end
    local summonConfig
    if dataParam.summonId then
  ...
  function unit:SaveValue_1052(value)
    if self.curValue_1052 and value > 0 then
      self.curValue_1052 = math.min(self.curValue_1052 + value, self.maxValue_1052)
      BattleBuffMgr.TriggerUnitListener(self, BUFF_DEDUCE_TYPE.SETTLE_EFFECT_1052_VALUE)
    end
  end
  
  function unit:ClearValue_1052()
    if self.curValue_1052 then
      self.curValue_1052 = 0
    end
  end
  ...
  function unit:CreateModel()
    ...
    if effect or singleEffect then
      ...
      ResourceManager.InstantiateAsync(EffectUtil.GetFullPath(effect), function(model)
        ...
        BattleResourceManager:AddGameObject(model, 0, 0, 0, scale, scale, scale)
        self:SetPosition()
        if self.summonFromUnitId == SPECIAL_CARD_OR_MONSTER.LASITE or self.summonFromUnitId == SPECIAL_CARD_OR_MONSTER.FEI then
          local summonFromUnit = BattleScene.GetUnitByUid(self.summonFromUnit)
  ...
  function unit:Update(isSecond)
    ...
    if self.summonFromUnitId == SPECIAL_CARD_OR_MONSTER.LING_YAO or self.summonFromUnitId == SPECIAL_CARD_OR_MONSTER.LASITE or self.summonFromUnitId == SPECIAL_CARD_OR_MONSTER.FEI then
      local fromUnit = BattleScene.GetUnitByUid(self.summonFromUnit)
      if fromUnit then
    ...
    if self.effectModel and (self.summonFromUnitId == SPECIAL_CARD_OR_MONSTER.LASITE or self.summonFromUnitId == SPECIAL_CARD_OR_MONSTER.FEI) and self.lastAttackFrame then
    ...
    if false == BattleMgr.isBattleServer then
      BattleAction.UpdateModelTimescale(self)
      if self.id == SPECIAL_CARD_OR_MONSTER.GUI_SHI or self.id == SPECIAL_CARD_OR_MONSTER.XIAO or self.id == SPECIAL_CARD_OR_MONSTER.SHOUHUZHE or self.id == SPECIAL_CARD_OR_MONSTER.FENGHEN or self.id == SPECIAL_CARD_OR_MONSTER.YONGYE or self.id == SPECIAL_CARD_OR_MONSTER.FEI then
  ...
  function unit:ChooseMoveTarget(onlyMove)
    ...
    if nil == moveTargetUid then
      ...
      if self.summonFromUnitId == SPECIAL_CARD_OR_MONSTER.LASITE or true == self.useSummonModelPosition then
  ...
  function unit:SaveDamaged(damage, fromOther, fromSummon, fromEnemy)
    if self.summonId and self.summonFromUnit then
      local summonUnit = BattleScene.GetUnitByUid(self.summonFromUnit)
      if summonUnit.isBuilding == false then
        summonUnit:SaveDamaged(damage, fromOther, true, fromEnemy)
        return
      end
    end
    if damage > 0 then
      ...
      if damage > 0 and self.isBuilding == false and self.summonId == nil and true ~= fromSummon then
        ...
        if fromEnemy then
          local value = self:GetEffectTotalValue(BUFF_EFFECT_ID.SPECIAL_1053)
          if value > 0 then
            local _, buffList = BattleBuffMgr.GetSettledBuffByUnitAndEffectID(self, BUFF_EFFECT_ID.SPECIAL_1053)
            local fromUnit = BattleScene.GetUnitByUid(buffList[1].fromUnitUid)
            if fromUnit then
              fromUnit:SaveValue_1052(math.ceil(damage * value / 10000))
            end
          end
        end
      end
