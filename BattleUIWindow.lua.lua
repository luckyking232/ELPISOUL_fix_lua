...
function BattleUIWindow.UpdateCard(unitUid, unit, barInfo)
  ...
  if unit then
    ...
    elseif unit.id == SPECIAL_CARD_OR_MONSTER.FEI then
      local cardSign = cardInfo.cardSign
      cardSign.c1Ctr.selectedIndex = 7
      local com = cardSign.Card10094
      if com then
        local count = #unit.cachedSummonUids
        local count2 = BattleBuffMgr.GetSettledBuffByUnitAndEffectID(unit, 1055, nil, true)
        com.NumberTxt.text = T(11046, count, count2)
        local progressBar = com.CardProgressBar
        local max = unit.maxValue_1052
        if 0 == max then
          progressBar.value = 0
        else
          progressBar.value = 100 * unit.curValue_1052 / max
        end
      end
    end
