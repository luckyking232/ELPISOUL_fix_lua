...
local PanDingKeZhi = function(atkUnit, defUnit, skill)
  ...
  for i = 1, #elementTypeAtk do
    kezhi = kezhi + defUnit:GetAttr(RestraintSubName[elementTypeAtk[i]]) + BattleBuffMgr.GetValueById(defUnit, BUFF_EFFECT_ID.ELEMENT_DAMAGE_RESTRAINT_ADD)
  end
  return kezhi
end
...
function BattleDataCount.GetSkillHurt(skillId, subSkillId, atkUnit, defUnit, bulletParams, hurtIndex, params)
  ...
  elseif mainSkillDamageRate > 0 and subSkillDamageRate > 0 then
    ...
    local directValue
    if realAtkUnit and (realAtkUnit.summonFromUnitId == SPECIAL_CARD_OR_MONSTER.LASITE or realAtkUnit.summonFromUnitId == SPECIAL_CARD_OR_MONSTER.FEI) then
      directValue = 0
    else
      ...
      if true == isContain then
        ...
        for i = 1, #effectList do
          ...
          if assistFromUnit:CanTargetChosen() then
            ...
            assistFromUnit:SaveDamaged(_buffEffectParams.hurt, nil, nil, atkUnit and atkUnit.camp == assistFromUnit.enemyCamp)
            if atkUnit then
        ...
        if #unitList > 1 then
          ...
          for _, unit in ipairs(unitList) do
            if unit.uid ~= defUnit.uid then
              ...
              unit:SaveDamaged(share_buffEffectParams.hurt, nil, nil, atkUnit and atkUnit.camp == unit.enemyCamp)
