  ...
  if BattleControl.isOver == false then
    ...
    if true ~= BattleMgr.isBattleServer then
      ...
      if BattleData.stageType == ProtoEnum.SCENE_TYPE.ARENA then
        ...
        local isOver = BattleScene.IsBattleOver()
        if isOver and BattleScene.IsBattleWin() ~= data.win then
          ActorService.ClientAbnormalReportReq("arena_" .. data.battleData.initData.seqId)
        end
        return
      end
