...
function Activity21ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity21ChallengeWindow.package, WinResConfig.Activity21ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1021_ChallengeWindowUis(contentPane)
...
function Activity21ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity21ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity21ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity21ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity21ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity21ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity21ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity21ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity21ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity21ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity21ChallengeWindow.ShopInfo()
...
function Activity21ChallengeWindow.GetChapterPassById(id)
...
function Activity21ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity21ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity21ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity21ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity21ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity21ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(2168))
end

function Activity21ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity21ChallengeWindow.name)
end

function Activity21ChallengeWindow.OnShown()
  if uis then
    Activity21ChallengeWindow.UpdateInfo()
  end
end

function Activity21ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity21ChallengeWindow
