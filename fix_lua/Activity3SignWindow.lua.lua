...
function Activity3SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity3SignWindow.package, WinResConfig.Activity3SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1002_SignWindowUis(contentPane)
    Activity3SignWindow.UpdateInfo()
    Activity3SignWindow.InitBtn()
  end)
end

function Activity3SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity3SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity3SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity3SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity3SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity3SignWindow.GetRewardData(groupId)
...
function Activity3SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity3SignWindow.CloseWindow)
end

function Activity3SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity3SignWindow.name)
end

function Activity3SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity3SignWindow
