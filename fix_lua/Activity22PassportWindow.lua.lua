...
function Activity22PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity22PassportWindow.package, WinResConfig.Activity22PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1022_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity22PassportWindow.name, {true})
      if passInfo then
        Activity22PassportWindow.InitPassport()
        Activity22PassportWindow.CheckItemTime()
      end
...
function Activity22PassportWindow.UpdateTextDisplay()
...
function Activity22PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity22PassportWindow.name,
        com = item,
...
function Activity22PassportWindow.InitEffect()
...
function Activity22PassportWindow.InitLv()
...
function Activity22PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity22PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity22PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity22PassportWindow.SaveLevel()
  local tb = {}
...
function Activity22PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity22PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity22PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity22PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity22PassportWindow.ShowReward()
  Activity22PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity22PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity22PassportWindow.ShowLevelUp()
end

function Activity22PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity22PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity22PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity22PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity22PassportWindow.RefreshRewardEnd(lv1, Activity22PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity22PassportWindow.RefreshRewardEnd(lv2, Activity22PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity22PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity22PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity22PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity22PassportWindow.name)
...
function Activity22PassportWindow.RefreshUI()
  Activity22PassportWindow.GetExpMxp()
  Activity22PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity22PassportWindow.GetExpMxp()
...
function Activity22PassportWindow.SetLevelInfo()
...
function Activity22PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity22PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity22PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity22PassportWindow.SetListPos()
  local list = uis.Main.RewardRegion.MiddleList.MiddleList
  local len = list.numItems
  for i = 1, len do
    if passInfo.lv == i - 3 or len - 3 <= passInfo.lv and i == passInfo.lv then
      list:ScrollToView(i - 1)
      Activity22PassportWindow.RefreshListReward(list.scrollPane.posX)
      break
    end
  end
...
function Activity22PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity22PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity22PassportWindow.RefreshRewardShow(data, objReward)
      Activity22PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity22PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity22PassportWindow.ShowGetEffect()
  local minScale = 0.82
  for i = 1, 2 do
    if animTween[i] then
...
function Activity22PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity22PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity22PassportWindow.IsReward(passId, lv)
...
function Activity22PassportWindow.RefreshRewardFirst()
  Activity22PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity22PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity22PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity22PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity22PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity22PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity22PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity22PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity22PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity22PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity22PassportWindow.RefreshRewardShow(data, Item)
...
function Activity22PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity22PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity22PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity22PassportWindow.name)
end

function Activity22PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity22PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity22PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity22PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity22BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity22PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity22PassportWindow.InitTask()
...
function Activity22PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity22PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity22PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity22PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity22PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity22PassportWindow.name) then
        local lvUp = Activity22PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity22PassportWindow.ShowExpBarAnim(para)
    else
      Activity22PassportWindow.SetLevelInfo()
    end
...
function Activity22PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity22PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity22PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity22PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity22PassportWindow
