...
function Activity7TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity7TaskWindow.package, WinResConfig.Activity7TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1006_TaskWindowUis(contentPane)
...
function Activity7TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity7TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity7TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity7TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity7TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity7TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity7TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity7TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(2015)
  title.TitleTxt.text = T(2014)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity7TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity7TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity7TaskWindow.name)
end

function Activity7TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity7TaskWindow
