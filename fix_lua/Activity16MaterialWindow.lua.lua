...
function Activity16MaterialWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity16MaterialWindow.package, WinResConfig.Activity16MaterialWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1016_MaterialWindowUis(contentPane)
...
function Activity16MaterialWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  maxCount = Activity16MaterialWindow.GetMaxCount()
  if activityInfo then
    local id = Activity16MaterialWindow.GetChapter()
    if id then
...
function Activity16MaterialWindow.GetChapter()
  for i, v in pairs(activityInfo.creamChapter) do
    return i
  end
end

function Activity16MaterialWindow.GetMaxCount()
...
function Activity16MaterialWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity16MaterialWindow.CloseWindow)
end

function Activity16MaterialWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity16MaterialWindow.name)
end

function Activity16MaterialWindow.OnShown()
  if uis then
    Activity16MaterialWindow.UpdateInfo()
  end
end

function Activity16MaterialWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity16MaterialWindow
