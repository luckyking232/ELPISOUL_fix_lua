    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity6_MiniGameService.MiniGameInfoReq(gameId, RefreshRewardList)
          end
...
function Activity6MiniGameTaskWindow.ReInitData()
end

function Activity6MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity6MiniGameTaskWindow.package, WinResConfig.Activity6MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1005_MiniTaskWindowUis(contentPane)
    Activity6MiniGameTaskWindow.UpdateInfo()
    Activity6MiniGameTaskWindow.InitBtn()
  end)
end

function Activity6MiniGameTaskWindow.UpdateInfo()
  local info = Activity6_MiniGameData.GetMiniGameInfo(gameId)
...
function Activity6MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity6MiniGameTaskWindow.name)
  end)
end

function Activity6MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity6MiniGameTaskWindow
