...
function Activity5PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity5PassportWindow.package, WinResConfig.Activity5PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1004_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity5PassportWindow.name, {true})
      if passInfo then
        Activity5PassportWindow.InitPassport()
        Activity5PassportWindow.CheckItemTime()
      end
...
function Activity5PassportWindow.UpdateTextDisplay()
...
function Activity5PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity5PassportWindow.name,
        com = item,
...
function Activity5PassportWindow.InitEffect()
...
function Activity5PassportWindow.InitLv()
...
function Activity5PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity5PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity5PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity5PassportWindow.SaveLevel()
  local tb = {}
...
function Activity5PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity5PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity5PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity5PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity5PassportWindow.ShowReward()
  Activity5PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity5PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity5PassportWindow.ShowLevelUp()
end

function Activity5PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity5PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity5PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity5PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity5PassportWindow.RefreshRewardEnd(lv1, Activity5PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity5PassportWindow.RefreshRewardEnd(lv2, Activity5PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity5PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity5PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity5PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity5PassportWindow.name)
...
function Activity5PassportWindow.RefreshUI()
  Activity5PassportWindow.GetExpMxp()
  Activity5PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity5PassportWindow.GetExpMxp()
...
function Activity5PassportWindow.SetLevelInfo()
...
function Activity5PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity5PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity5PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity5PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity5PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity5PassportWindow.RefreshRewardShow(data, objReward)
      Activity5PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity5PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity5PassportWindow.ShowGetEffect()
  local minScale = 0.82
  for i = 1, 2 do
    if animTween[i] then
      ...
      for k, v in ipairs(animTween[i]) do
        TimerUtil.setTimeout(0.2 + k * 0.1, function()
          if 1 == v.type then
            UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/PassCard/FX_ui_passcard_linyao_light.prefab", v.reward.parent:GetChild("Effect1Holder"), nil, true, function()
            ...
            UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/PassCard/FX_ui_passcard_linyao_light_down.prefab", v.reward.parent:GetChild("Effect2Holder"), nil, true, function()
              UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/PassCard/FX_passcard_get_light.prefab", v.reward:GetChild("EffectHolder"))
              animTween[i][k] = nil
            end)
          end
...
function Activity5PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity5PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity5PassportWindow.IsReward(passId, lv)
...
function Activity5PassportWindow.RefreshRewardFirst()
  Activity5PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity5PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity5PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity5PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity5PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity5PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity5PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity5PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity5PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity5PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity5PassportWindow.RefreshRewardShow(data, Item)
...
function Activity5PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity5PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity5PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity5PassportWindow.name)
end

function Activity5PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity5PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity5PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity5PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity5BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity5PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity5PassportWindow.InitTask()
...
function Activity5PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity5PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity5PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity5PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity5PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity5PassportWindow.name) then
        local lvUp = Activity5PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity5PassportWindow.ShowExpBarAnim(para)
    else
      Activity5PassportWindow.SetLevelInfo()
    end
...
function Activity5PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity5PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity5PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity5PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity5PassportWindow
