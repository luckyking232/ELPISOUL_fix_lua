...
function Activity7DungeonWindow.ReInitData()
end

function Activity7DungeonWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity7DungeonWindow.package, WinResConfig.Activity7DungeonWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1006_ActivityDungeonWindowUis(contentPane)
    ...
    if plotShowId then
      ...
      if reviewWord then
        reviewWord.onClick:Set(function()
          ...
          if id then
            OpenWindow(WinResConfig.Activity7PlotWindow.name, nil, id, configData)
          end
        end)
...
function Activity7DungeonWindow.LoadBg()
  ...
  local spine = LuaUtil.FindChild(effect, "eventcovers_0007", true)
  local bg1 = LuaUtil.FindChild(effect, "eventcovers_0007_BG_1", true)
  local bg2 = LuaUtil.FindChild(effect, "eventcovers_0007_BG_2", true)
  local spineTb = {
    spine,
    bg1,
    bg2
...
function Activity7DungeonWindow.InitRedDot()
  RedDotMgr.AddNode({
    windowName = WinResConfig.Activity7DungeonWindow.name,
...
function Activity7DungeonWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
    ...
    if configData.boss_chapter_ids then
      ...
      if 2 == #arr then
        ...
        uis.Main.BossBtn.onClick:Set(function()
          if lock then
            OpenWindow(WinResConfig.Activity7BossBattleWindow.name)
          else
            FloatTipsUtil.ShowWarnTips(tips)
          end
        end)
...
function Activity7DungeonWindow.InitBtnTxt()
  UIUtil.SetText(uis.Main.BossBtn, T(1775))
  UIUtil.SetText(uis.Main.MaterialBtn, T(1776))
  uis.Main.NormalBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity7ChallengeWindow.name)
  end)
  UIUtil.SetText(uis.Main.NormalBtn, T(1777))
    ...
    if passData then
      uis.Main.PassBtn.onClick:Set(function()
        PassportService.GetBattlePassInfoReq(function()
          ...
          OpenWindow(WinResConfig.Activity7PassportWindow.name)
        end)
      end)
  ...
  UIUtil.SetText(uis.Main.PlotBtn, T(1772))
  uis.Main.ShopBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity7ShopWindow.name)
  end)
  UIUtil.SetText(uis.Main.ShopBtn, T(1774))
  uis.Main.SignBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity7SignWindow.name)
  end)
  UIUtil.SetText(uis.Main.SignBtn, T(1778))
  uis.Main.TaskBtn.onClick:Set(function()
    ActivityDungeonService.GetActivityAllReq(function()
      OpenWindow(WinResConfig.Activity7TaskWindow.name)
    end)
  end)
  UIUtil.SetText(uis.Main.TaskBtn, T(1779))
  uis.Main.MiniGameBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity7MiniGameMainWindow.name)
  end)
end

function Activity7DungeonWindow.InitBtn()
  jumpTb = CurrencyReturnWindow.SetCurrencyReturn(WinResConfig.Activity7DungeonWindow.name, uis.Main.CurrencyReturn, FEATURE_ENUM.HOME_ADVENTURE)
end

function Activity7DungeonWindow.OnShown()
  if uis then
    Activity7DungeonWindow.UpdateInfo()
  end
end

function Activity7DungeonWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity7DungeonWindow.name)
  uis = nil
  if effect then
...
function Activity7DungeonWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_CROSS_DAY then
    ActivityDungeonMgr.CheckActivityEnd()
  end
end

return Activity7DungeonWindow
