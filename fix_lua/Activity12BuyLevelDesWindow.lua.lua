...
function Activity12BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity12BuyLevelDesWindow.package, WinResConfig.Activity12BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1012_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity12BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity12BuyLevelDesWindow.ShowAssetsTips()
...
function Activity12BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity12BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity12BuyLevelDesWindow.GetGold(buyNum)
...
function Activity12BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity12BuyLevelDesWindow.ShowReward()
  local data = Activity12BuyLevelDesWindow.GetRewardItem()
...
function Activity12BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity12BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity12BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity12BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity12BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity12BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity12PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity12BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity12PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity12BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity12BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity12BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity12BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity12BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity12BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity12BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity12BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity12BuyLevelDesWindow
