    ...
    Activity12_MiniGameService.MiniGameInfoReq(function()
      RedDotMgr.AddNode({
        ...
        visibleFunc = function()
          return RedDotActivityDungeon.MiniGame12_TaskRewardable() or RedDotActivityDungeon.MiniGame12_DailyTaskRewardable()
        end,
        dataType = RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON
      })
      RedDotMgr.UpdateNodeByWindowName(WinResConfig.Activity12DungeonWindow.name)
    end)
  end
end

function Activity12DungeonWindow.ReInitData()
end

function Activity12DungeonWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity12DungeonWindow.package, WinResConfig.Activity12DungeonWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1012_ActivityDungeonWindowUis(contentPane)
    ...
    if plotShowId then
      ...
      if reviewWord then
        reviewWord.onClick:Set(function()
          ...
          if id then
            OpenWindow(WinResConfig.Activity12PlotWindow.name, nil, id, configData)
          end
        end)
...
function Activity12DungeonWindow.LoadBg()
  ...
  local spine = LuaUtil.FindChild(effect, "eventcovers_0012_1", true)
  local bg1 = LuaUtil.FindChild(effect, "eventcovers_0012_2", true)
  local bg2 = LuaUtil.FindChild(effect, "eventcovers_0012_3", true)
  local spineTb = {
    spine,
    bg1,
    bg2
...
function Activity12DungeonWindow.InitRedDot()
  RedDotMgr.AddNode({
    windowName = WinResConfig.Activity12DungeonWindow.name,
...
function Activity12DungeonWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
    ...
    if configData.boss_chapter_ids then
      ...
      if 2 == #arr then
        ...
        uis.Main.BossBtn.onClick:Set(function()
          if lock then
            OpenWindow(WinResConfig.Activity12BossBattleWindow.name)
          else
            FloatTipsUtil.ShowWarnTips(tips)
          end
        end)
...
function Activity12DungeonWindow.InitBtnTxt()
  UIUtil.SetText(uis.Main.BossBtn, T(1928))
  UIUtil.SetText(uis.Main.MaterialBtn, T(1929))
  uis.Main.NormalBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity12ChallengeWindow.name)
  end)
  UIUtil.SetText(uis.Main.NormalBtn, T(1930))
    ...
    if passData then
      uis.Main.PassBtn.onClick:Set(function()
        PassportService.GetBattlePassInfoReq(function()
          ...
          OpenWindow(WinResConfig.Activity12PassportWindow.name)
        end)
      end)
      UIUtil.SetText(uis.Main.PassBtn, passData.name())
    end
  end
  local conf = TableData.GetConfig(70441014, "BaseActivityStageGame")
  if conf then
  ...
  UIUtil.SetText(uis.Main.PlotBtn, T(1925))
  uis.Main.ShopBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity12ShopWindow.name)
  end)
  UIUtil.SetText(uis.Main.ShopBtn, T(1927))
  uis.Main.SignBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity12SignWindow.name)
  end)
  UIUtil.SetText(uis.Main.SignBtn, T(1931))
  uis.Main.TaskBtn.onClick:Set(function()
    ActivityDungeonService.GetActivityAllReq(function()
      OpenWindow(WinResConfig.Activity12TaskWindow.name)
    end)
  end)
  UIUtil.SetText(uis.Main.TaskBtn, T(1926))
  uis.Main.MiniGameBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity12MiniGameMainWindow.name)
  end)
end

function Activity12DungeonWindow.InitBtn()
  jumpTb = CurrencyReturnWindow.SetCurrencyReturn(WinResConfig.Activity12DungeonWindow.name, uis.Main.CurrencyReturn, FEATURE_ENUM.HOME_ADVENTURE)
end

function Activity12DungeonWindow.OnShown()
  if uis then
    Activity12DungeonWindow.UpdateInfo()
  end
end

function Activity12DungeonWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity12DungeonWindow.name)
  uis = nil
  if effect then
...
function Activity12DungeonWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_CROSS_DAY then
    ActivityDungeonMgr.CheckActivityEnd()
  end
end

return Activity12DungeonWindow
