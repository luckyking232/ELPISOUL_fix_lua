...
function Activity19BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity19BuyLevelDesWindow.package, WinResConfig.Activity19BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1019_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity19BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity19BuyLevelDesWindow.ShowAssetsTips()
...
function Activity19BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity19BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity19BuyLevelDesWindow.GetGold(buyNum)
...
function Activity19BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity19BuyLevelDesWindow.ShowReward()
  local data = Activity19BuyLevelDesWindow.GetRewardItem()
...
function Activity19BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity19BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity19BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity19BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity19BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity19BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity19PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity19BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity19PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity19BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity19BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity19BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity19BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity19BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity19BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity19BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity19BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity19BuyLevelDesWindow
