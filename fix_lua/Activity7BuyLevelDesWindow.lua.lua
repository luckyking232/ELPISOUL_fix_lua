...
function Activity7BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity7BuyLevelDesWindow.package, WinResConfig.Activity7BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1006_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity7BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity7BuyLevelDesWindow.ShowAssetsTips()
...
function Activity7BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity7BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity7BuyLevelDesWindow.GetGold(buyNum)
...
function Activity7BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity7BuyLevelDesWindow.ShowReward()
  local data = Activity7BuyLevelDesWindow.GetRewardItem()
...
function Activity7BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity7BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity7BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity7BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity7BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity7BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity7PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity7BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity7PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity7BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity7BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity7BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity7BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity7BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity7BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity7BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity7BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity7BuyLevelDesWindow
