...
function Activity3ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity3ShopWindow.package, WinResConfig.Activity3ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1002_ShopWindowUis(contentPane)
    Activity3ShopWindow.UpdateInfo()
    Activity3ShopWindow.InitBtn()
  end)
end

function Activity3ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity3ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity3ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity3ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity3ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity3ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity3ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity3ShopWindow.GetShopGrid(groupId)
...
function Activity3ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity3ShopWindow.CloseWindow)
end

function Activity3ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity3ShopWindow.name)
end

function Activity3ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity3ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity3ShopWindow.UpdateInfo(true)
  end
end

return Activity3ShopWindow
