...
function ActivityCasket3ExplainTipsWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.ActivityCasket3ExplainTipsWindow.package, WinResConfig.ActivityCasket3ExplainTipsWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityCasket02_ExplainTipsWindowUis(contentPane)
...
function ActivityCasket3ExplainTipsWindow.UpdateInfo()
...
function ActivityCasket3ExplainTipsWindow.ShowShop()
...
function ActivityCasket3ExplainTipsWindow.ShowRule()
...
function ActivityCasket3ExplainTipsWindow.ShowRecord()
  ...
  local data = ActivityCasket3ExplainTipsWindow.GetCost(turnTableConfig.group_id)
  ...
  function tips.ContentList.itemRenderer(i, item)
    if data[i + 1].reward then
      ...
      if arr[1] then
        ...
        local poolInfo = ActivityCasket3ExplainTipsWindow.GetPoolInfo(turnTableInfo.pools, data[i + 1].id)
        if poolInfo then
...
function ActivityCasket3ExplainTipsWindow.GetPoolInfo(pools, id)
...
function ActivityCasket3ExplainTipsWindow.GetCost(groupId)
...
function ActivityCasket3ExplainTipsWindow.InitBtn()
  ...
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.ActivityCasket3ExplainTipsWindow.name)
  end)
end

function ActivityCasket3ExplainTipsWindow.OnClose()
  uis = nil
  contentPane = nil
  turnTableConfig = nil
  turnTableInfo = nil
...
function ActivityCasket3ExplainTipsWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.ActivityCasket.REFRESH_PAY_SHOP then
    ActivityCasket3ExplainTipsWindow.ShowShop()
  end
end

return ActivityCasket3ExplainTipsWindow
