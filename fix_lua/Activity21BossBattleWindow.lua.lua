...
function Activity21BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity21BossBattleWindow.package, WinResConfig.Activity21BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1021_BossBattleWindowUis(contentPane)
    Activity21BossBattleWindow.UpdateInfo()
    Activity21BossBattleWindow.InitBtn()
  end)
end

function Activity21BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity21BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity21BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity21BossBattleWindow.FormatRemainTime(time)
...
function Activity21BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity21BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity21BossBattleWindow.CloseWindow)
end

function Activity21BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity21BossBattleWindow.name)
end

function Activity21BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity21BossBattleWindow
