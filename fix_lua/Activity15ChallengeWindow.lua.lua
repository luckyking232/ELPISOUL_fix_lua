...
function Activity15ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity15ChallengeWindow.package, WinResConfig.Activity15ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1015_ChallengeWindowUis(contentPane)
...
function Activity15ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity15ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity15ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity15ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity15ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity15ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity15ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity15ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity15ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity15ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity15ChallengeWindow.ShopInfo()
...
function Activity15ChallengeWindow.GetChapterPassById(id)
...
function Activity15ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity15ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity15ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity15ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity15ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity15ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(2009))
end

function Activity15ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity15ChallengeWindow.name)
end

function Activity15ChallengeWindow.OnShown()
  if uis then
    Activity15ChallengeWindow.UpdateInfo()
  end
end

function Activity15ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity15ChallengeWindow
