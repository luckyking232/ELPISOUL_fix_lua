    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity4_MiniGameService.MiniGameInfoReq(RefreshRewardList)
          end
...
function Activity4MiniGameTaskWindow.ReInitData()
end

function Activity4MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity4MiniGameTaskWindow.package, WinResConfig.Activity4MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1003_MiniTaskWindowUis(contentPane)
    Activity4MiniGameTaskWindow.UpdateInfo()
    Activity4MiniGameTaskWindow.InitBtn()
  end)
end

function Activity4MiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity4_MiniGameData.GetMiniGameInfo()
...
function Activity4MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity4MiniGameTaskWindow.name)
  end)
end

function Activity4MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity4MiniGameTaskWindow
