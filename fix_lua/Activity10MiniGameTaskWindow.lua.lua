    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity10_MiniGameService.MiniGameInfoReq(RefreshRewardList)
          end
...
function Activity10MiniGameTaskWindow.ReInitData()
end

function Activity10MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity10MiniGameTaskWindow.package, WinResConfig.Activity10MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1010_MiniTaskWindowUis(contentPane)
    Activity10MiniGameTaskWindow.UpdateInfo()
    Activity10MiniGameTaskWindow.InitBtn()
  end)
end

function Activity10MiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity10_MiniGameData.GetMiniGameInfo()
...
function Activity10MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity10MiniGameTaskWindow.name)
  end)
end

function Activity10MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity10MiniGameTaskWindow
