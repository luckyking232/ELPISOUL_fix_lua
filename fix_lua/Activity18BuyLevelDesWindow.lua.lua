...
function Activity18BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity18BuyLevelDesWindow.package, WinResConfig.Activity18BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1018_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity18BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity18BuyLevelDesWindow.ShowAssetsTips()
...
function Activity18BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity18BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity18BuyLevelDesWindow.GetGold(buyNum)
...
function Activity18BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity18BuyLevelDesWindow.ShowReward()
  local data = Activity18BuyLevelDesWindow.GetRewardItem()
...
function Activity18BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity18BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity18BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity18BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity18BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity18BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity18PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity18BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity18PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity18BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity18BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity18BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity18BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity18BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity18BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity18BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity18BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity18BuyLevelDesWindow
