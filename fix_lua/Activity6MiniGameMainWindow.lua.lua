require("ActivityDungeon1005_MiniGameWindowByName")
local Activity6MiniGameMainWindow = {}
local uis, contentPane
local gameId = 70441007
ld("Activity6_MiniGame")
local RefreshPanelInfo = function()
  local highestScoreTxt = T(20620)
  local dailyScoreTxt = T(20485)
  local info = Activity6_MiniGameData.GetMiniGameInfo(gameId)
  local conf = TableData.GetConfig(gameId, "BaseActivityStageGame")
  local str = conf.game_day_reward[1]
  local splits = Split(str, ":")
  local itemType, itemId, count = tonumber(splits[1]), tonumber(splits[2]), tonumber(splits[3])
  uis.Main.Top.Integral.WordTxt.text = highestScoreTxt
  uis.Main.Top.Integral.NumberTxt.text = info and info.miniHighScore or 0
  local rewarded = info.miniDailyStat >= ProtoEnum.TASK_STATE.REWARD
  local finished = info.miniDailyStat >= ProtoEnum.TASK_STATE.FINISHED
  uis.Main.Botton.TodayTask.c1Ctr.selectedIndex = not finished and 0 or rewarded and 2 or 1
  uis.Main.Botton.TodayTask.Item.c1Ctr.selectedIndex = 0
  local achieveDailyScoreThresholdTxt = T(20612, finished and 1 or 0)
  uis.Main.Botton.TodayTask.WordTxt.text = achieveDailyScoreThresholdTxt
  UIUtil.SetText(uis.Main.StartBtn, T(20630), "WordTxt")
  UIUtil.ShowItemFrame(itemId, uis.Main.Botton.TodayTask.Item.ItemFrame.root, count, nil, function(context)
    UIUtil.CommonItemClickCallback(itemId, itemType)
    context:StopPropagation()
  end)
  uis.Main.Botton.TodayTask.root.onClick:Set(function()
    if finished and not rewarded then
      Activity6_MiniGameService.MiniGameGetDailyRewardReq(gameId)
    end
  end)
end

function Activity6MiniGameMainWindow.ReInitData()
end

function Activity6MiniGameMainWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity6MiniGameMainWindow.package, WinResConfig.Activity6MiniGameMainWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1005_MiniGameWindowUis(contentPane)
    Activity6MiniGameMainWindow.UpdateInfo()
    Activity6MiniGameMainWindow.InitBtn()
  end)
end

function Activity6MiniGameMainWindow.UpdateInfo()
  uis.Main.BackGround.BackGroundLoader.url = "ui://ActivityDungeon1005/MiniGame_1000"
  RedDotMgr.AddNode({
    windowName = WinResConfig.Activity6MiniGameMainWindow.name,
    com = uis.Main.Botton.TodayTask.root,
    visibleFunc = function()
      return RedDotActivityDungeon.MiniGame6_DailyTaskRewardable()
    end,
    dataType = RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON
  })
  RedDotMgr.AddNode({
    windowName = WinResConfig.Activity6MiniGameMainWindow.name,
    com = uis.Main.Botton.TaskBtn,
    visibleFunc = function()
      return RedDotActivityDungeon.MiniGame6_TaskRewardable()
    end,
    dataType = RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON
  })
  Activity6_MiniGameService.MiniGameInfoReq(gameId)
end

function Activity6MiniGameMainWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity6MiniGameMainWindow.name)
  end)
  uis.Main.StartBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity6MiniGameWindow.name)
  end)
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity6MiniGameMainWindow.name)
  end)
  uis.Main.StartBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity6MiniGameChoiceWindow.name)
  end)
  uis.Main.Botton.RecordBtn.onClick:Set(function()
    local info = Activity6_MiniGameData.GetMiniGameInfo(gameId)
    local records = info.records
    if not records or not _G.next(records) then
      FloatTipsUtil.ShowWarnTips(T(20619))
      return
    end
    OpenWindow(WinResConfig.Activity6MiniGameRecordWindow.name)
  end)
  uis.Main.Botton.TaskBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity6MiniGameTaskWindow.name)
  end)
end

function Activity6MiniGameMainWindow.OnClose()
  uis = nil
  contentPane = nil
end

function Activity6MiniGameMainWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Activity6_MiniGame.REFRESH_MINIGAME_INFO then
    RefreshPanelInfo()
  end
end

return Activity6MiniGameMainWindow
