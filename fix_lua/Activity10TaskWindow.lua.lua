...
function Activity10TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity10TaskWindow.package, WinResConfig.Activity10TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1010_TaskWindowUis(contentPane)
...
function Activity10TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity10TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity10TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity10TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity10TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity10TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity10TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity10TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(1892)
  title.TitleTxt.text = T(1891)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity10TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity10TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity10TaskWindow.name)
end

function Activity10TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity10TaskWindow
