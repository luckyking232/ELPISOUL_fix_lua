...
function Activity16BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity16BuyLevelDesWindow.package, WinResConfig.Activity16BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1016_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity16BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity16BuyLevelDesWindow.ShowAssetsTips()
...
function Activity16BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity16BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity16BuyLevelDesWindow.GetGold(buyNum)
...
function Activity16BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity16BuyLevelDesWindow.ShowReward()
  local data = Activity16BuyLevelDesWindow.GetRewardItem()
...
function Activity16BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity16BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity16BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity16BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity16BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity16BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity16PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity16BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity16PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity16BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity16BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity16BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity16BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity16BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity16BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity16BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity16BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity16BuyLevelDesWindow
