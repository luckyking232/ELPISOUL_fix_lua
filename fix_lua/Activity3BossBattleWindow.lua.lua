...
function Activity3BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity3BossBattleWindow.package, WinResConfig.Activity3BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1002_BossBattleWindowUis(contentPane)
    Activity3BossBattleWindow.UpdateInfo()
    Activity3BossBattleWindow.InitBtn()
  end)
end

function Activity3BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity3BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity3BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity3BossBattleWindow.FormatRemainTime(time)
...
function Activity3BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity3BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity3BossBattleWindow.CloseWindow)
end

function Activity3BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity3BossBattleWindow.name)
end

function Activity3BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity3BossBattleWindow
