...
function Activity19MaterialWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity19MaterialWindow.package, WinResConfig.Activity19MaterialWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1019_MaterialWindowUis(contentPane)
...
function Activity19MaterialWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  maxCount = Activity19MaterialWindow.GetMaxCount()
  if activityInfo then
    local id = Activity19MaterialWindow.GetChapter()
    if id then
...
function Activity19MaterialWindow.GetChapter()
  for i, v in pairs(activityInfo.creamChapter) do
    return i
  end
end

function Activity19MaterialWindow.GetMaxCount()
...
function Activity19MaterialWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity19MaterialWindow.CloseWindow)
end

function Activity19MaterialWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity19MaterialWindow.name)
end

function Activity19MaterialWindow.OnShown()
  if uis then
    Activity19MaterialWindow.UpdateInfo()
  end
end

function Activity19MaterialWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity19MaterialWindow
