...
function Activity14SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity14SignWindow.package, WinResConfig.Activity14SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1014_SignWindowUis(contentPane)
    Activity14SignWindow.UpdateInfo()
    Activity14SignWindow.InitBtn()
  end)
end

function Activity14SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity14SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity14SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity14SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity14SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity14SignWindow.GetRewardData(groupId)
...
function Activity14SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity14SignWindow.CloseWindow)
end

function Activity14SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity14SignWindow.name)
end

function Activity14SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity14SignWindow
