...
function Activity10BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity10BossBattleWindow.package, WinResConfig.Activity10BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1010_BossBattleWindowUis(contentPane)
    Activity10BossBattleWindow.UpdateInfo()
    Activity10BossBattleWindow.InitBtn()
  end)
end

function Activity10BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity10BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity10BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity10BossBattleWindow.FormatRemainTime(time)
...
function Activity10BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity10BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity10BossBattleWindow.CloseWindow)
end

function Activity10BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity10BossBattleWindow.name)
end

function Activity10BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity10BossBattleWindow
