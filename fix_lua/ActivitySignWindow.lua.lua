...
function ActivitySignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.ActivitySignWindow.package, WinResConfig.ActivitySignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1_SignWindowUis(contentPane)
    ActivitySignWindow.UpdateInfo()
    ActivitySignWindow.InitBtn()
  end)
end

function ActivitySignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = ActivitySignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  ActivitySignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          ActivitySignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function ActivitySignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function ActivitySignWindow.GetRewardData(groupId)
...
function ActivitySignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(ActivitySignWindow.CloseWindow)
end

function ActivitySignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.ActivitySignWindow.name)
end

function ActivitySignWindow.OnClose()
  uis = nil
  contentPane = nil
end

return ActivitySignWindow
