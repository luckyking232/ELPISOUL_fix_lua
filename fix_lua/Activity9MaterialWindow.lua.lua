...
function Activity9MaterialWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity9MaterialWindow.package, WinResConfig.Activity9MaterialWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1008_MaterialWindowUis(contentPane)
...
function Activity9MaterialWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  maxCount = Activity9MaterialWindow.GetMaxCount()
  if activityInfo then
    local id = Activity9MaterialWindow.GetChapter()
    if id then
...
function Activity9MaterialWindow.GetChapter()
  for i, v in pairs(activityInfo.creamChapter) do
    return i
  end
end

function Activity9MaterialWindow.GetMaxCount()
...
function Activity9MaterialWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity9MaterialWindow.CloseWindow)
end

function Activity9MaterialWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity9MaterialWindow.name)
end

function Activity9MaterialWindow.OnShown()
  if uis then
    Activity9MaterialWindow.UpdateInfo()
  end
end

function Activity9MaterialWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity9MaterialWindow
