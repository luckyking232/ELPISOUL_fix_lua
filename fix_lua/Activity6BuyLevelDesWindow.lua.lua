...
function Activity6BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity6BuyLevelDesWindow.package, WinResConfig.Activity6BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1005_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity6BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity6BuyLevelDesWindow.ShowAssetsTips()
...
function Activity6BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity6BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity6BuyLevelDesWindow.GetGold(buyNum)
...
function Activity6BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity6BuyLevelDesWindow.ShowReward()
  local data = Activity6BuyLevelDesWindow.GetRewardItem()
...
function Activity6BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity6BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity6BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity6BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity6BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity6BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity6PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity6BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity6PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity6BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity6BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity6BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity6BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity6BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity6BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity6BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity6BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity6BuyLevelDesWindow
