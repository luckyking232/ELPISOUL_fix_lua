...
function Activity13PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity13PassportWindow.package, WinResConfig.Activity13PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1013_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity13PassportWindow.name, {true})
      if passInfo then
        Activity13PassportWindow.InitPassport()
        Activity13PassportWindow.CheckItemTime()
      end
...
function Activity13PassportWindow.UpdateTextDisplay()
...
function Activity13PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity13PassportWindow.name,
        com = item,
...
function Activity13PassportWindow.InitEffect()
...
function Activity13PassportWindow.InitLv()
...
function Activity13PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity13PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity13PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity13PassportWindow.SaveLevel()
  local tb = {}
...
function Activity13PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity13PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity13PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity13PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity13PassportWindow.ShowReward()
  Activity13PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity13PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity13PassportWindow.ShowLevelUp()
end

function Activity13PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity13PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity13PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity13PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity13PassportWindow.RefreshRewardEnd(lv1, Activity13PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity13PassportWindow.RefreshRewardEnd(lv2, Activity13PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity13PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity13PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity13PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity13PassportWindow.name)
...
function Activity13PassportWindow.RefreshUI()
  Activity13PassportWindow.GetExpMxp()
  Activity13PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity13PassportWindow.GetExpMxp()
...
function Activity13PassportWindow.SetLevelInfo()
...
function Activity13PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity13PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity13PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity13PassportWindow.SetListPos()
  local list = uis.Main.RewardRegion.MiddleList.MiddleList
  local len = list.numItems
  for i = 1, len do
    if passInfo.lv == i - 3 or len - 3 <= passInfo.lv and i == passInfo.lv then
      list:ScrollToView(i - 1)
      Activity13PassportWindow.RefreshListReward(list.scrollPane.posX)
      break
    end
  end
...
function Activity13PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity13PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity13PassportWindow.RefreshRewardShow(data, objReward)
      Activity13PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity13PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity13PassportWindow.ShowGetEffect()
  local minScale = 0.82
  for i = 1, 2 do
    if animTween[i] then
...
function Activity13PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity13PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity13PassportWindow.IsReward(passId, lv)
...
function Activity13PassportWindow.RefreshRewardFirst()
  Activity13PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity13PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity13PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity13PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity13PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity13PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity13PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity13PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity13PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity13PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity13PassportWindow.RefreshRewardShow(data, Item)
...
function Activity13PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity13PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity13PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity13PassportWindow.name)
end

function Activity13PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity13PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity13PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity13PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity13BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity13PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity13PassportWindow.InitTask()
...
function Activity13PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity13PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity13PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity13PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity13PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity13PassportWindow.name) then
        local lvUp = Activity13PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity13PassportWindow.ShowExpBarAnim(para)
    else
      Activity13PassportWindow.SetLevelInfo()
    end
...
function Activity13PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity13PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity13PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity13PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity13PassportWindow
