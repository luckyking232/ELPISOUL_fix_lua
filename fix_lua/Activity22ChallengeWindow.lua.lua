...
function Activity22ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity22ChallengeWindow.package, WinResConfig.Activity22ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1022_ChallengeWindowUis(contentPane)
...
function Activity22ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity22ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity22ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity22ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity22ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity22ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity22ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity22ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity22ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity22ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity22ChallengeWindow.ShopInfo()
...
function Activity22ChallengeWindow.GetChapterPassById(id)
...
function Activity22ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity22ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity22ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity22ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity22ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity22ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(2182))
end

function Activity22ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity22ChallengeWindow.name)
end

function Activity22ChallengeWindow.OnShown()
  if uis then
    Activity22ChallengeWindow.UpdateInfo()
  end
end

function Activity22ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity22ChallengeWindow
