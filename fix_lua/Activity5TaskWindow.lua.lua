...
function Activity5TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity5TaskWindow.package, WinResConfig.Activity5TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1004_TaskWindowUis(contentPane)
...
function Activity5TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity5TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity5TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity5TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity5TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity5TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity5TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity5TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(1701)
  title.TitleTxt.text = T(1700)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity5TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity5TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity5TaskWindow.name)
end

function Activity5TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity5TaskWindow
