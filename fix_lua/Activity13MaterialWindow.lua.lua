...
function Activity13MaterialWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity13MaterialWindow.package, WinResConfig.Activity13MaterialWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1013_MaterialWindowUis(contentPane)
...
function Activity13MaterialWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  maxCount = Activity13MaterialWindow.GetMaxCount()
  if activityInfo then
    local id = Activity13MaterialWindow.GetChapter()
    if id then
...
function Activity13MaterialWindow.GetChapter()
  for i, v in pairs(activityInfo.creamChapter) do
    return i
  end
end

function Activity13MaterialWindow.GetMaxCount()
...
function Activity13MaterialWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity13MaterialWindow.CloseWindow)
end

function Activity13MaterialWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity13MaterialWindow.name)
end

function Activity13MaterialWindow.OnShown()
  if uis then
    Activity13MaterialWindow.UpdateInfo()
  end
end

function Activity13MaterialWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity13MaterialWindow
