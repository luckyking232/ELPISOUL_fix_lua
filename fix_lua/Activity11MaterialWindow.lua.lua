...
function Activity11MaterialWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity11MaterialWindow.package, WinResConfig.Activity11MaterialWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1011_MaterialWindowUis(contentPane)
...
function Activity11MaterialWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  maxCount = Activity11MaterialWindow.GetMaxCount()
  if activityInfo then
    local id = Activity11MaterialWindow.GetChapter()
    if id then
...
function Activity11MaterialWindow.GetChapter()
  for i, v in pairs(activityInfo.creamChapter) do
    return i
  end
end

function Activity11MaterialWindow.GetMaxCount()
...
function Activity11MaterialWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity11MaterialWindow.CloseWindow)
end

function Activity11MaterialWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity11MaterialWindow.name)
end

function Activity11MaterialWindow.OnShown()
  if uis then
    Activity11MaterialWindow.UpdateInfo()
  end
end

function Activity11MaterialWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity11MaterialWindow
