...
function Activity12SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity12SignWindow.package, WinResConfig.Activity12SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1012_SignWindowUis(contentPane)
    Activity12SignWindow.UpdateInfo()
    Activity12SignWindow.InitBtn()
  end)
end

function Activity12SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity12SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity12SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity12SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity12SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity12SignWindow.GetRewardData(groupId)
...
function Activity12SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity12SignWindow.CloseWindow)
end

function Activity12SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity12SignWindow.name)
end

function Activity12SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity12SignWindow
