...
function Activity11TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity11TaskWindow.package, WinResConfig.Activity11TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1011_TaskWindowUis(contentPane)
...
function Activity11TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity11TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity11TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity11TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity11TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity11TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity11TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity11TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(1919)
  title.TitleTxt.text = T(1918)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity11TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity11TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity11TaskWindow.name)
end

function Activity11TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity11TaskWindow
