...
function Activity14TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity14TaskWindow.package, WinResConfig.Activity14TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1014_TaskWindowUis(contentPane)
...
function Activity14TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity14TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity14TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask(ActivityDungeonData.GetShowId()) and true or false
  end
end

function Activity14TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity14TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity14TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity14TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity14TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(2006)
  title.TitleTxt.text = T(2005)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity14TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity14TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity14TaskWindow.name)
end

function Activity14TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity14TaskWindow
