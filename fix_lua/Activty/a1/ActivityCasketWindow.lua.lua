...
function ActivityCasketWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.ActivityCasketWindow.package, WinResConfig.ActivityCasketWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityCasket_CasketWindowUis(contentPane)
...
function ActivityCasketWindow.GetPoolInfo(pools, id)
...
function ActivityCasketWindow.UpdateInfo(poolId)
  ...
  if turnTableConfig then
    if nil == mapCom then
      mapCom = UIMgr:CreateObject("ActivityCasket", "TagRegion_01")
      uis.Main.TagRegion.root:AddChild(mapCom)
    end
    allData = ActivityCasketWindow.GetData(turnTableConfig.group_id)
    maxNum = #allData
    for i = 1, maxNum do
      local tips = mapCom:GetChild("Tag" .. i)
      local arr = GetConfigItemList(allData[i].reward, true)
      local poolInfo = ActivityCasketWindow.GetPoolInfo(turnTableInfo.pools, allData[i].id)
      if tips and arr[1] then
        if poolId and poolInfo and poolInfo.poolId == poolId then
          ...
          if arr[1].type == ProtoEnum.TUPLE_TYPE.T_FASHION then
            ...
            UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/turntable/FX_ui_turntable_midlight_open_get.prefab", tempHolder, nil, true)
          else
        ...
        if arr[1].type == ProtoEnum.TUPLE_TYPE.T_FASHION then
          ...
          if nil == boxObj then
            boxObj = UIUtil.SetEffectToUI("Assets/Art/Models/UI_spine/prefab/ui_Turntable_skin_gift.prefab", tips:GetChild("BoxHolder"))
          end
  ...
  if maxNum and turnTableInfo and turnTableInfo.turnNum and turnTableInfo.turnNum >= maxNum then
    ...
    local cost = ActivityCasketWindow.GetCost()
    ...
    uis.Main.GaChaBtn.onClick:Set(function()
      ...
      if canDo then
        ...
        SignService.ActivityDoTurnTableReq(turnTableInfo.baseInfo.activityId, function(msg)
          ...
          animPlay = true
          ActivityCasketWindow.PlanAnim(tempPools, msg.poolId, function()
            if uis then
              uis.Main.GaChaBtn.touchable = true
              ActivityCasketWindow.UpdateInfo(msg.poolId)
...
function ActivityCasketWindow.PlanAnim(tempPools, poolId, callback)
  curIndex = curIndex + 1
  local indexTb = {}
  local tempInd
  for i = 1, #allData do
    local poolInfo = ActivityCasketWindow.GetPoolInfo(tempPools, allData[i].id)
  ...
  for i = 1, len do
    ...
    TimerUtil.setTimeout(i * time, function()
      if uis then
        ...
        if 10 == newTb[i] then
          SoundUtil.PlayUISfx("event:/sfx/sfx_ui/ui_sys/ui_sfx_act9_rotary_reward_box")
          UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/turntable/FX_ui_turntable_midlight_open.prefab", tips:GetChild("EffectHolder"), nil, true)
        else
        ...
        if i == len then
          if curIndex <= tatalNum then
            LeanTween.delayedCall(time, function()
              ActivityCasketWindow.PlanAnim(tempPools, poolId, callback)
            end)
...
function ActivityCasketWindow.GetData(groupId)
...
function ActivityCasketWindow.GetCost()
...
function ActivityCasketWindow.InitAssetsTips()
  ...
  function uis.Main.AssetsTipsList.itemRenderer(i, item)
    ...
    item:GetChild("AssetsBtn").onClick:Set(function()
      uis.Main.AssetsTipsList.onClick:Set(function()
        OpenWindow(WinResConfig.ActivityCasketExplainTipsWindow.name, nil, 0, turnTableConfig, turnTableInfo.baseInfo.activityId)
      end)
    end)
...
function ActivityCasketWindow.InitBtn()
  local btnTb = {
    [1] = {
  ...
  for i = 1, #btnTb do
    ...
    btnTb[i].btn.onClick:Set(function()
      OpenWindow(WinResConfig.ActivityCasketExplainTipsWindow.name, nil, btnTb[i].ctr, turnTableConfig, turnTableInfo.baseInfo.activityId)
    end)
  end
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.ActivityCasketWindow.name)
  end)
  UIUtil.SetText(uis.Main.ShopBtn, T(1877), "TagTxt")
end

function ActivityCasketWindow.OnShown()
  if uis then
    ActivityCasketWindow.UpdateInfo()
    showBoxOpen = nil
  end
...
function ActivityCasketWindow.OnClose()
  uis = nil
  contentPane = nil
  turnTableConfig = nil
  turnTableInfo = nil
...
function ActivityCasketWindow.CheckActivityEnd()
...
function ActivityCasketWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_CROSS_DAY then
    ActivityCasketWindow.CheckActivityEnd()
  end
end

return ActivityCasketWindow
