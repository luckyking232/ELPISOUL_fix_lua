    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity13_MiniGameService.MiniGameInfoReq(gameId, RefreshRewardList)
          end
...
function Activity13BounceMiniGameTaskWindow.ReInitData()
end

function Activity13BounceMiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity13BounceMiniGameTaskWindow.package, WinResConfig.Activity13BounceMiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1013_MiniTaskWindowUis(contentPane)
    Activity13BounceMiniGameTaskWindow.UpdateInfo()
    Activity13BounceMiniGameTaskWindow.InitBtn()
  end)
end

function Activity13BounceMiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity13_MiniGameData.GetMiniGameInfo(gameId)
...
function Activity13BounceMiniGameTaskWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity13BounceMiniGameTaskWindow.name)
  end)
...
function Activity13BounceMiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity13BounceMiniGameTaskWindow
