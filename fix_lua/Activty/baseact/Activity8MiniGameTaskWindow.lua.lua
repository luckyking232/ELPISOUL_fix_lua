    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity8_MiniGameService.MiniGameInfoReq(RefreshRewardList)
          end
...
function Activity8MiniGameTaskWindow.ReInitData()
end

function Activity8MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity8MiniGameTaskWindow.package, WinResConfig.Activity8MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1007_MiniTaskWindowUis(contentPane)
    Activity8MiniGameTaskWindow.UpdateInfo()
    Activity8MiniGameTaskWindow.InitBtn()
  end)
end

function Activity8MiniGameTaskWindow.UpdateInfo()
  local info = Activity8_MiniGameData.GetMiniGameInfo(gameId)
...
function Activity8MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity8MiniGameTaskWindow.name)
  end)
end

function Activity8MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity8MiniGameTaskWindow
