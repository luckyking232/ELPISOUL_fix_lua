...
function Activity16TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity16TaskWindow.package, WinResConfig.Activity16TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1016_TaskWindowUis(contentPane)
...
function Activity16TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity16TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity16TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask(ActivityDungeonData.GetShowId()) and true or false
  end
end

function Activity16TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity16TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity16TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity16TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity16TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(2059)
  title.TitleTxt.text = T(2058)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity16TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity16TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity16TaskWindow.name)
end

function Activity16TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity16TaskWindow
