...
function Activity16ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity16ShopWindow.package, WinResConfig.Activity16ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1016_ShopWindowUis(contentPane)
    Activity16ShopWindow.UpdateInfo()
    Activity16ShopWindow.InitBtn()
  end)
end

function Activity16ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity16ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity16ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity16ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity16ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity16ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity16ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity16ShopWindow.GetShopGrid(groupId)
...
function Activity16ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity16ShopWindow.CloseWindow)
end

function Activity16ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity16ShopWindow.name)
end

function Activity16ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity16ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity16ShopWindow.UpdateInfo(true)
  end
end

return Activity16ShopWindow
