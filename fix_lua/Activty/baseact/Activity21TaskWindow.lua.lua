...
function Activity21TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity21TaskWindow.package, WinResConfig.Activity21TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1021_TaskWindowUis(contentPane)
...
function Activity21TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity21TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity21TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask(ActivityDungeonData.GetShowId()) and true or false
  end
end

function Activity21TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity21TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity21TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity21TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity21TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(2174)
  title.TitleTxt.text = T(2173)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity21TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity21TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity21TaskWindow.name)
end

function Activity21TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity21TaskWindow
