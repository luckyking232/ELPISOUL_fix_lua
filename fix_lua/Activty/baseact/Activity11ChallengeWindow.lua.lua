...
function Activity11ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity11ChallengeWindow.package, WinResConfig.Activity11ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1011_ChallengeWindowUis(contentPane)
...
function Activity11ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity11ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity11ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity11ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity11ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity11ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity11ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity11ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity11ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity11ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity11ChallengeWindow.ShopInfo()
...
function Activity11ChallengeWindow.GetChapterPassById(id)
...
function Activity11ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity11ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity11ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity11ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity11ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity11ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(1913))
end

function Activity11ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity11ChallengeWindow.name)
end

function Activity11ChallengeWindow.OnShown()
  if uis then
    Activity11ChallengeWindow.UpdateInfo()
  end
end

function Activity11ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity11ChallengeWindow
