...
local InitMiniGameInfo = function()
  ...
  if data.id == 70440014 then
    ...
    Activity14_MiniGameService.MiniGameInfoReq(gameId, function()
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity14DungeonWindow.name,
        com = uis.Main.MiniGameBtn,
        visibleFunc = function()
          return RedDotActivityDungeon.MiniGame14_TaskRewardable() or RedDotActivityDungeon.MiniGame14_DailyTaskRewardable()
        end,
        dataType = RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON
      })
      RedDotMgr.UpdateNodeByWindowName(WinResConfig.Activity14DungeonWindow.name)
    end)
  end
end

function Activity14DungeonWindow.ReInitData()
end

function Activity14DungeonWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity14DungeonWindow.package, WinResConfig.Activity14DungeonWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1014_ActivityDungeonWindowUis(contentPane)
    ...
    if plotShowId then
      ...
      if reviewWord then
        reviewWord.onClick:Set(function()
          ...
          if id then
            OpenWindow(WinResConfig.Activity14PlotWindow.name, nil, id, configData)
          end
        end)
...
function Activity14DungeonWindow.LoadBg()
...
function Activity14DungeonWindow.InitRedDot()
  RedDotMgr.AddNode({
    windowName = WinResConfig.Activity14DungeonWindow.name,
...
function Activity14DungeonWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
    ...
    if configData.boss_chapter_ids then
      ...
      if 2 == #arr then
        ...
        uis.Main.BossBtn.onClick:Set(function()
          if lock then
            OpenWindow(WinResConfig.Activity14BossBattleWindow.name)
          else
            FloatTipsUtil.ShowWarnTips(tips)
          end
        end)
...
function Activity14DungeonWindow.InitBtnTxt()
  UIUtil.SetText(uis.Main.BossBtn, T(2001))
  UIUtil.SetText(uis.Main.MaterialBtn, T(2002))
  uis.Main.NormalBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity14ChallengeWindow.name)
  end)
  UIUtil.SetText(uis.Main.NormalBtn, T(2003))
    ...
    if passData then
      uis.Main.PassBtn.onClick:Set(function()
        PassportService.GetBattlePassInfoReq(function()
          ...
          OpenWindow(WinResConfig.Activity14PassportWindow.name)
        end)
      end)
  ...
  UIUtil.SetText(uis.Main.PlotBtn, T(1998))
  uis.Main.ShopBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity14ShopWindow.name)
  end)
  UIUtil.SetText(uis.Main.ShopBtn, T(2000))
  uis.Main.SignBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity14SignWindow.name)
  end)
  UIUtil.SetText(uis.Main.SignBtn, T(2004))
  uis.Main.TaskBtn.onClick:Set(function()
    ActivityDungeonService.GetActivityAllReq(function()
      OpenWindow(WinResConfig.Activity14TaskWindow.name)
    end)
  end)
  UIUtil.SetText(uis.Main.TaskBtn, T(1999))
  uis.Main.MiniGameBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity14MiniGameMainWindow.name)
  end)
end

function Activity14DungeonWindow.InitBtn()
  jumpTb = CurrencyReturnWindow.SetCurrencyReturn(WinResConfig.Activity14DungeonWindow.name, uis.Main.CurrencyReturn, FEATURE_ENUM.HOME_ADVENTURE)
end

function Activity14DungeonWindow.OnShown()
  if uis then
    Activity14DungeonWindow.UpdateInfo()
  end
end

function Activity14DungeonWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity14DungeonWindow.name)
  uis = nil
  if effect then
...
function Activity14DungeonWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_CROSS_DAY then
    ActivityDungeonMgr.CheckActivityEnd()
  end
end

return Activity14DungeonWindow
