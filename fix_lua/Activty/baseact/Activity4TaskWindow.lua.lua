...
function Activity4TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity4TaskWindow.package, WinResConfig.Activity4TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1003_TaskWindowUis(contentPane)
...
function Activity4TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity4TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity4TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity4TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity4TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity4TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity4TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity4TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(1674)
  title.TitleTxt.text = T(1673)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity4TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity4TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity4TaskWindow.name)
end

function Activity4TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity4TaskWindow
