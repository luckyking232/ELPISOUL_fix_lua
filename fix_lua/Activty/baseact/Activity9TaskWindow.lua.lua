...
function Activity9TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity9TaskWindow.package, WinResConfig.Activity9TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1008_TaskWindowUis(contentPane)
...
function Activity9TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity9TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity9TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity9TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity9TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity9TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity9TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity9TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(1847)
  title.TitleTxt.text = T(1846)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity9TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity9TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity9TaskWindow.name)
end

function Activity9TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity9TaskWindow
