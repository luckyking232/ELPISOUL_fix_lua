...
function Activity21ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity21ShopWindow.package, WinResConfig.Activity21ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1021_ShopWindowUis(contentPane)
    Activity21ShopWindow.UpdateInfo()
    Activity21ShopWindow.InitBtn()
  end)
end

function Activity21ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity21ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity21ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity21ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity21ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity21ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity21ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity21ShopWindow.GetShopGrid(groupId)
...
function Activity21ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity21ShopWindow.CloseWindow)
end

function Activity21ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity21ShopWindow.name)
end

function Activity21ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity21ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity21ShopWindow.UpdateInfo(true)
  end
end

return Activity21ShopWindow
