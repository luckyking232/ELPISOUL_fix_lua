...
function Activity5BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity5BossBattleWindow.package, WinResConfig.Activity5BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1004_BossBattleWindowUis(contentPane)
    Activity5BossBattleWindow.UpdateInfo()
    Activity5BossBattleWindow.InitBtn()
  end)
end

function Activity5BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity5BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity5BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity5BossBattleWindow.FormatRemainTime(time)
...
function Activity5BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity5BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity5BossBattleWindow.CloseWindow)
end

function Activity5BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity5BossBattleWindow.name)
end

function Activity5BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity5BossBattleWindow
