...
function Activity22ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity22ShopWindow.package, WinResConfig.Activity22ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1022_ShopWindowUis(contentPane)
    Activity22ShopWindow.UpdateInfo()
    Activity22ShopWindow.InitBtn()
  end)
end

function Activity22ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity22ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity22ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity22ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity22ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity22ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity22ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity22ShopWindow.GetShopGrid(groupId)
...
function Activity22ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity22ShopWindow.CloseWindow)
end

function Activity22ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity22ShopWindow.name)
end

function Activity22ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity22ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity22ShopWindow.UpdateInfo(true)
  end
end

return Activity22ShopWindow
