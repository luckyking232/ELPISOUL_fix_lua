...
function Activity8ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity8ChallengeWindow.package, WinResConfig.Activity8ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1007_ChallengeWindowUis(contentPane)
...
function Activity8ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity8ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity8ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity8ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity8ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity8ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity8ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity8ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity8ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity8ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity8ChallengeWindow.ShopInfo()
...
function Activity8ChallengeWindow.GetChapterPassById(id)
...
function Activity8ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity8ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity8ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity8ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity8ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity8ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(1813))
end

function Activity8ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity8ChallengeWindow.name)
end

function Activity8ChallengeWindow.OnShown()
  if uis then
    Activity8ChallengeWindow.UpdateInfo()
  end
end

function Activity8ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity8ChallengeWindow
