...
function Activity6BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity6BossBattleWindow.package, WinResConfig.Activity6BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1005_BossBattleWindowUis(contentPane)
    Activity6BossBattleWindow.UpdateInfo()
    Activity6BossBattleWindow.InitBtn()
  end)
end

function Activity6BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity6BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity6BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity6BossBattleWindow.FormatRemainTime(time)
...
function Activity6BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity6BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity6BossBattleWindow.CloseWindow)
end

function Activity6BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity6BossBattleWindow.name)
end

function Activity6BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity6BossBattleWindow
