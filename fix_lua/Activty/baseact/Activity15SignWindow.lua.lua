...
function Activity15SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity15SignWindow.package, WinResConfig.Activity15SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1015_SignWindowUis(contentPane)
    Activity15SignWindow.UpdateInfo()
    Activity15SignWindow.InitBtn()
  end)
end

function Activity15SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity15SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity15SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity15SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity15SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity15SignWindow.GetRewardData(groupId)
...
function Activity15SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity15SignWindow.CloseWindow)
end

function Activity15SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity15SignWindow.name)
end

function Activity15SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity15SignWindow
