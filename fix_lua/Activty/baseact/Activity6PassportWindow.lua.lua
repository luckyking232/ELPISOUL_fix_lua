...
function Activity6PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity6PassportWindow.package, WinResConfig.Activity6PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1005_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity6PassportWindow.name, {true})
      if passInfo then
        Activity6PassportWindow.InitPassport()
        Activity6PassportWindow.CheckItemTime()
      end
...
function Activity6PassportWindow.UpdateTextDisplay()
...
function Activity6PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity6PassportWindow.name,
        com = item,
...
function Activity6PassportWindow.InitEffect()
...
function Activity6PassportWindow.InitLv()
...
function Activity6PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity6PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity6PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity6PassportWindow.SaveLevel()
  local tb = {}
...
function Activity6PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity6PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity6PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity6PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity6PassportWindow.ShowReward()
  Activity6PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity6PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity6PassportWindow.ShowLevelUp()
end

function Activity6PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity6PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity6PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity6PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity6PassportWindow.RefreshRewardEnd(lv1, Activity6PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity6PassportWindow.RefreshRewardEnd(lv2, Activity6PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity6PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity6PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity6PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity6PassportWindow.name)
...
function Activity6PassportWindow.RefreshUI()
  Activity6PassportWindow.GetExpMxp()
  Activity6PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity6PassportWindow.GetExpMxp()
...
function Activity6PassportWindow.SetLevelInfo()
...
function Activity6PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity6PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity6PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity6PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity6PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity6PassportWindow.RefreshRewardShow(data, objReward)
      Activity6PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity6PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity6PassportWindow.ShowGetEffect()
  for i = 1, 2 do
    if animTween[i] then
      ...
      for k, v in ipairs(animTween[i]) do
        TimerUtil.setTimeout(0.2 + k * 0.1, function()
          if 1 == v.type then
            UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/PassCard/FX_ui_passcard_luoya_light_up.prefab", v.reward.parent:GetChild("Effect1Holder"), nil, true, function()
            ...
            UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/PassCard/FX_ui_passcard_luoya_light_down.prefab", v.reward.parent:GetChild("Effect2Holder"), nil, true, function()
              UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/PassCard/FX_passcard_get_light.prefab", v.reward:GetChild("EffectHolder"))
              animTween[i][k] = nil
            end)
          end
...
function Activity6PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity6PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity6PassportWindow.IsReward(passId, lv)
...
function Activity6PassportWindow.RefreshRewardFirst()
  Activity6PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity6PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity6PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity6PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity6PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity6PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity6PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity6PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity6PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity6PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity6PassportWindow.RefreshRewardShow(data, Item)
...
function Activity6PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity6PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity6PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity6PassportWindow.name)
end

function Activity6PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity6PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity6PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity6PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity6BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity6PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity6PassportWindow.InitTask()
...
function Activity6PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity6PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity6PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity6PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity6PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity6PassportWindow.name) then
        local lvUp = Activity6PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity6PassportWindow.ShowExpBarAnim(para)
    else
      Activity6PassportWindow.SetLevelInfo()
    end
...
function Activity6PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity6PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity6PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity6PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity6PassportWindow
