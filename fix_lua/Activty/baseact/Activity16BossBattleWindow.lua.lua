...
function Activity16BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity16BossBattleWindow.package, WinResConfig.Activity16BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1016_BossBattleWindowUis(contentPane)
    Activity16BossBattleWindow.UpdateInfo()
    Activity16BossBattleWindow.InitBtn()
  end)
end

function Activity16BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity16BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity16BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity16BossBattleWindow.FormatRemainTime(time)
...
function Activity16BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity16BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity16BossBattleWindow.CloseWindow)
end

function Activity16BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity16BossBattleWindow.name)
end

function Activity16BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity16BossBattleWindow
