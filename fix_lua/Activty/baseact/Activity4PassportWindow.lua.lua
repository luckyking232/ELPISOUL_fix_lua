...
function Activity4PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity4PassportWindow.package, WinResConfig.Activity4PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1003_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity4PassportWindow.name, {true})
      if passInfo then
        Activity4PassportWindow.InitPassport()
        Activity4PassportWindow.CheckItemTime()
      end
...
function Activity4PassportWindow.UpdateTextDisplay()
...
function Activity4PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity4PassportWindow.name,
        com = item,
...
function Activity4PassportWindow.InitEffect()
...
function Activity4PassportWindow.InitLv()
...
function Activity4PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity4PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity4PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity4PassportWindow.SaveLevel()
  local tb = {}
...
function Activity4PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity4PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity4PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity4PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity4PassportWindow.ShowReward()
  Activity4PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity4PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity4PassportWindow.ShowLevelUp()
end

function Activity4PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity4PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity4PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity4PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity4PassportWindow.RefreshRewardEnd(lv1, Activity4PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity4PassportWindow.RefreshRewardEnd(lv2, Activity4PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity4PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity4PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity4PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity4PassportWindow.name)
...
function Activity4PassportWindow.RefreshUI()
  Activity4PassportWindow.GetExpMxp()
  Activity4PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity4PassportWindow.GetExpMxp()
...
function Activity4PassportWindow.SetLevelInfo()
...
function Activity4PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity4PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity4PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity4PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity4PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity4PassportWindow.RefreshRewardShow(data, objReward)
      Activity4PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity4PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity4PassportWindow.ShowGetEffect()
  local minScale = 0.82
  for i = 1, 2 do
    if animTween[i] then
...
function Activity4PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity4PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity4PassportWindow.IsReward(passId, lv)
...
function Activity4PassportWindow.RefreshRewardFirst()
  Activity4PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity4PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity4PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity4PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity4PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity4PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity4PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity4PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity4PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity4PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity4PassportWindow.RefreshRewardShow(data, Item)
...
function Activity4PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity4PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity4PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity4PassportWindow.name)
end

function Activity4PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity4PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity4PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity4PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity4BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity4PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity4PassportWindow.InitTask()
...
function Activity4PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity4PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity4PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity4PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity4PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity4PassportWindow.name) then
        local lvUp = Activity4PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity4PassportWindow.ShowExpBarAnim(para)
    else
      Activity4PassportWindow.SetLevelInfo()
    end
...
function Activity4PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity4PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity4PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity4PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity4PassportWindow
