...
function Activity22SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity22SignWindow.package, WinResConfig.Activity22SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1022_SignWindowUis(contentPane)
    Activity22SignWindow.UpdateInfo()
    Activity22SignWindow.InitBtn()
  end)
end

function Activity22SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity22SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity22SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity22SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity22SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity22SignWindow.GetRewardData(groupId)
...
function Activity22SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity22SignWindow.CloseWindow)
end

function Activity22SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity22SignWindow.name)
end

function Activity22SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity22SignWindow
