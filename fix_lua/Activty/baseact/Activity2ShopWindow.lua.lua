...
function Activity2ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity2ShopWindow.package, WinResConfig.Activity2ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1001_ShopWindowUis(contentPane)
    Activity2ShopWindow.UpdateInfo()
    Activity2ShopWindow.InitBtn()
  end)
end

function Activity2ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity2ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity2ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity2ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity2ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity2ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity2ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity2ShopWindow.GetShopGrid(groupId)
...
function Activity2ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity2ShopWindow.CloseWindow)
end

function Activity2ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity2ShopWindow.name)
end

function Activity2ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity2ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity2ShopWindow.UpdateInfo(true)
  end
end

return Activity2ShopWindow
