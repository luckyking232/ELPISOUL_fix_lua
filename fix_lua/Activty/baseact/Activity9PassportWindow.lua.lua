...
function Activity9PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity9PassportWindow.package, WinResConfig.Activity9PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1008_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity9PassportWindow.name, {true})
      if passInfo then
        Activity9PassportWindow.InitPassport()
        Activity9PassportWindow.CheckItemTime()
      end
...
function Activity9PassportWindow.UpdateTextDisplay()
...
function Activity9PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity9PassportWindow.name,
        com = item,
...
function Activity9PassportWindow.InitEffect()
...
function Activity9PassportWindow.InitLv()
...
function Activity9PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity9PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity9PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity9PassportWindow.SaveLevel()
  local tb = {}
...
function Activity9PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity9PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity9PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity9PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity9PassportWindow.ShowReward()
  Activity9PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity9PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity9PassportWindow.ShowLevelUp()
end

function Activity9PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity9PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity9PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity9PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity9PassportWindow.RefreshRewardEnd(lv1, Activity9PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity9PassportWindow.RefreshRewardEnd(lv2, Activity9PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity9PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity9PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity9PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity9PassportWindow.name)
...
function Activity9PassportWindow.RefreshUI()
  Activity9PassportWindow.GetExpMxp()
  Activity9PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity9PassportWindow.GetExpMxp()
...
function Activity9PassportWindow.SetLevelInfo()
...
function Activity9PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity9PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity9PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity9PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity9PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity9PassportWindow.RefreshRewardShow(data, objReward)
      Activity9PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity9PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity9PassportWindow.ShowGetEffect()
  local minScale = 0.82
  for i = 1, 2 do
    if animTween[i] then
...
function Activity9PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity9PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity9PassportWindow.IsReward(passId, lv)
...
function Activity9PassportWindow.RefreshRewardFirst()
  Activity9PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity9PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity9PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity9PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity9PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity9PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity9PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity9PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity9PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity9PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity9PassportWindow.RefreshRewardShow(data, Item)
...
function Activity9PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity9PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity9PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity9PassportWindow.name)
end

function Activity9PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity9PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity9PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity9PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity9BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity9PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity9PassportWindow.InitTask()
...
function Activity9PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity9PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity9PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity9PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity9PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity9PassportWindow.name) then
        local lvUp = Activity9PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity9PassportWindow.ShowExpBarAnim(para)
    else
      Activity9PassportWindow.SetLevelInfo()
    end
...
function Activity9PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity9PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity9PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity9PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity9PassportWindow
