...
function Activity4ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity4ShopWindow.package, WinResConfig.Activity4ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1003_ShopWindowUis(contentPane)
    Activity4ShopWindow.UpdateInfo()
    Activity4ShopWindow.InitBtn()
  end)
end

function Activity4ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity4ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity4ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity4ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity4ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity4ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity4ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity4ShopWindow.GetShopGrid(groupId)
...
function Activity4ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity4ShopWindow.CloseWindow)
end

function Activity4ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity4ShopWindow.name)
end

function Activity4ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity4ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity4ShopWindow.UpdateInfo(true)
  end
end

return Activity4ShopWindow
