...
function Activity8TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity8TaskWindow.package, WinResConfig.Activity8TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1007_TaskWindowUis(contentPane)
...
function Activity8TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity8TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity8TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity8TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity8TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity8TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity8TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity8TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(1819)
  title.TitleTxt.text = T(1818)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity8TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity8TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity8TaskWindow.name)
end

function Activity8TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity8TaskWindow
