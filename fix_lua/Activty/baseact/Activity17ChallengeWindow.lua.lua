...
function Activity17ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity17ChallengeWindow.package, WinResConfig.Activity17ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1017_ChallengeWindowUis(contentPane)
...
function Activity17ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity17ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity17ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity17ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity17ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity17ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity17ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity17ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity17ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity17ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity17ChallengeWindow.ShopInfo()
...
function Activity17ChallengeWindow.GetChapterPassById(id)
...
function Activity17ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity17ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity17ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity17ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity17ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity17ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(2081))
end

function Activity17ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity17ChallengeWindow.name)
end

function Activity17ChallengeWindow.OnShown()
  if uis then
    Activity17ChallengeWindow.UpdateInfo()
  end
end

function Activity17ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity17ChallengeWindow
