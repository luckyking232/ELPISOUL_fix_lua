...
function Activity14ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity14ChallengeWindow.package, WinResConfig.Activity14ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1014_ChallengeWindowUis(contentPane)
...
function Activity14ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity14ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity14ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity14ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity14ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity14ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity14ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity14ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity14ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity14ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity14ChallengeWindow.ShopInfo()
...
function Activity14ChallengeWindow.GetChapterPassById(id)
...
function Activity14ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity14ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity14ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity14ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity14ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity14ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(2000))
end

function Activity14ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity14ChallengeWindow.name)
end

function Activity14ChallengeWindow.OnShown()
  if uis then
    Activity14ChallengeWindow.UpdateInfo()
  end
end

function Activity14ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity14ChallengeWindow
