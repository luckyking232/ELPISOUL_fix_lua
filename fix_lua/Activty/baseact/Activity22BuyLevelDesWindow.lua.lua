...
function Activity22BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity22BuyLevelDesWindow.package, WinResConfig.Activity22BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1022_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity22BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity22BuyLevelDesWindow.ShowAssetsTips()
...
function Activity22BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity22BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity22BuyLevelDesWindow.GetGold(buyNum)
...
function Activity22BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity22BuyLevelDesWindow.ShowReward()
  local data = Activity22BuyLevelDesWindow.GetRewardItem()
...
function Activity22BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity22BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity22BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity22BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity22BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity22BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity22PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity22BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity22PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity22BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity22BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity22BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity22BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity22BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity22BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity22BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity22BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity22BuyLevelDesWindow
