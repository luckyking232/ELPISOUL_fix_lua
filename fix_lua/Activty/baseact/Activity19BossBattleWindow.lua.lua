...
function Activity19BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity19BossBattleWindow.package, WinResConfig.Activity19BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1019_BossBattleWindowUis(contentPane)
    Activity19BossBattleWindow.UpdateInfo()
    Activity19BossBattleWindow.InitBtn()
  end)
end

function Activity19BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity19BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity19BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity19BossBattleWindow.FormatRemainTime(time)
...
function Activity19BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity19BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity19BossBattleWindow.CloseWindow)
end

function Activity19BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity19BossBattleWindow.name)
end

function Activity19BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity19BossBattleWindow
