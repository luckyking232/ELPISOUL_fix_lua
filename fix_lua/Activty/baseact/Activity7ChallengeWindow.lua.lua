...
function Activity7ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity7ChallengeWindow.package, WinResConfig.Activity7ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1006_ChallengeWindowUis(contentPane)
...
function Activity7ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity7ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity7ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity7ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity7ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity7ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity7ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity7ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity7ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity7ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity7ChallengeWindow.ShopInfo()
...
function Activity7ChallengeWindow.GetChapterPassById(id)
...
function Activity7ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity7ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity7ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity7ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity7ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity7ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(1774))
end

function Activity7ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity7ChallengeWindow.name)
end

function Activity7ChallengeWindow.OnShown()
  if uis then
    Activity7ChallengeWindow.UpdateInfo()
  end
end

function Activity7ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity7ChallengeWindow
