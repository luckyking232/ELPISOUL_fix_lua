    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity20_MiniGameService.MiniGameInfoReq(RefreshRewardList)
          end
...
function Activity20MiniGameTaskWindow.ReInitData()
end

function Activity20MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity20MiniGameTaskWindow.package, WinResConfig.Activity20MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1020_MiniTaskWindowUis(contentPane)
    Activity20MiniGameTaskWindow.UpdateInfo()
    Activity20MiniGameTaskWindow.InitBtn()
  end)
end

function Activity20MiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity20_MiniGameData.GetMiniGameInfo()
...
function Activity20MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity20MiniGameTaskWindow.name)
  end)
end

function Activity20MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity20MiniGameTaskWindow
