...
function Activity17ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity17ShopWindow.package, WinResConfig.Activity17ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1017_ShopWindowUis(contentPane)
    Activity17ShopWindow.UpdateInfo()
    Activity17ShopWindow.InitBtn()
  end)
end

function Activity17ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity17ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity17ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity17ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity17ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity17ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity17ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity17ShopWindow.GetShopGrid(groupId)
...
function Activity17ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity17ShopWindow.CloseWindow)
end

function Activity17ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity17ShopWindow.name)
end

function Activity17ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity17ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity17ShopWindow.UpdateInfo(true)
  end
end

return Activity17ShopWindow
