...
function Activity18ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity18ChallengeWindow.package, WinResConfig.Activity18ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1018_ChallengeWindowUis(contentPane)
...
function Activity18ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity18ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity18ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity18ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity18ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity18ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity18ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity18ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity18ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity18ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity18ChallengeWindow.ShopInfo()
...
function Activity18ChallengeWindow.GetChapterPassById(id)
...
function Activity18ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity18ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity18ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity18ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity18ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity18ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(2109))
end

function Activity18ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity18ChallengeWindow.name)
end

function Activity18ChallengeWindow.OnShown()
  if uis then
    Activity18ChallengeWindow.UpdateInfo()
  end
end

function Activity18ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity18ChallengeWindow
