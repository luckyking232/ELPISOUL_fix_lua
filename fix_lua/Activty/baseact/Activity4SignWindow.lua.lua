...
function Activity4SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity4SignWindow.package, WinResConfig.Activity4SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1003_SignWindowUis(contentPane)
    Activity4SignWindow.UpdateInfo()
    Activity4SignWindow.InitBtn()
  end)
end

function Activity4SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity4SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity4SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity4SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity4SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity4SignWindow.GetRewardData(groupId)
...
function Activity4SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity4SignWindow.CloseWindow)
end

function Activity4SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity4SignWindow.name)
end

function Activity4SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity4SignWindow
