...
function Activity16ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity16ChallengeWindow.package, WinResConfig.Activity16ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1016_ChallengeWindowUis(contentPane)
...
function Activity16ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity16ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity16ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity16ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity16ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity16ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity16ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity16ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity16ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity16ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity16ChallengeWindow.ShopInfo()
...
function Activity16ChallengeWindow.GetChapterPassById(id)
...
function Activity16ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity16ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity16ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity16ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity16ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity16ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(2053))
end

function Activity16ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity16ChallengeWindow.name)
end

function Activity16ChallengeWindow.OnShown()
  if uis then
    Activity16ChallengeWindow.UpdateInfo()
  end
end

function Activity16ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity16ChallengeWindow
