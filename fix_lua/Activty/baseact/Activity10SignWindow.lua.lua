...
function Activity10SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity10SignWindow.package, WinResConfig.Activity10SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1010_SignWindowUis(contentPane)
    Activity10SignWindow.UpdateInfo()
    Activity10SignWindow.InitBtn()
  end)
end

function Activity10SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity10SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity10SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity10SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity10SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity10SignWindow.GetRewardData(groupId)
...
function Activity10SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity10SignWindow.CloseWindow)
end

function Activity10SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity10SignWindow.name)
end

function Activity10SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity10SignWindow
