    ...
    Activity16_MiniGameService.MiniGameInfoReq(function()
      RedDotMgr.AddNode({
        ...
        visibleFunc = function()
          return RedDotActivityDungeon.MiniGame16_TaskRewardable() or RedDotActivityDungeon.MiniGame16_DailyTaskRewardable()
        end,
        dataType = RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON
      })
      RedDotMgr.UpdateNodeByWindowName(WinResConfig.Activity16DungeonWindow.name)
    end)
  end
end

function Activity16DungeonWindow.ReInitData()
end

function Activity16DungeonWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity16DungeonWindow.package, WinResConfig.Activity16DungeonWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1016_ActivityDungeonWindowUis(contentPane)
    ...
    if plotShowId then
      ...
      if reviewWord then
        reviewWord.onClick:Set(function()
          ...
          if id then
            OpenWindow(WinResConfig.Activity16PlotWindow.name, nil, id, configData)
          end
        end)
...
function Activity16DungeonWindow.LoadBg()
...
function Activity16DungeonWindow.InitRedDot()
  RedDotMgr.AddNode({
    windowName = WinResConfig.Activity16DungeonWindow.name,
...
function Activity16DungeonWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
    ...
    if configData.boss_chapter_ids then
      ...
      if 2 == #arr then
        ...
        uis.Main.BossBtn.onClick:Set(function()
          if lock then
            OpenWindow(WinResConfig.Activity16BossBattleWindow.name)
          else
            FloatTipsUtil.ShowWarnTips(tips)
          end
        end)
...
function Activity16DungeonWindow.InitBtnTxt()
  UIUtil.SetText(uis.Main.BossBtn, T(2054))
  UIUtil.SetText(uis.Main.MaterialBtn, T(2055))
  uis.Main.NormalBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity16ChallengeWindow.name)
  end)
  UIUtil.SetText(uis.Main.NormalBtn, T(2056))
    ...
    if passData then
      uis.Main.PassBtn.onClick:Set(function()
        PassportService.GetBattlePassInfoReq(function()
          ...
          OpenWindow(WinResConfig.Activity16PassportWindow.name)
        end)
      end)
  ...
  UIUtil.SetText(uis.Main.PlotBtn, T(2051))
  uis.Main.ShopBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity16ShopWindow.name)
  end)
  UIUtil.SetText(uis.Main.ShopBtn, T(2053))
  uis.Main.SignBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity16SignWindow.name)
  end)
  UIUtil.SetText(uis.Main.SignBtn, T(2057))
  uis.Main.TaskBtn.onClick:Set(function()
    ActivityDungeonService.GetActivityAllReq(function()
      OpenWindow(WinResConfig.Activity16TaskWindow.name)
    end)
  end)
  UIUtil.SetText(uis.Main.TaskBtn, T(2052))
  uis.Main.MiniGameBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity16MiniGameMainWindow.name)
  end)
end

function Activity16DungeonWindow.InitBtn()
  jumpTb = CurrencyReturnWindow.SetCurrencyReturn(WinResConfig.Activity16DungeonWindow.name, uis.Main.CurrencyReturn, FEATURE_ENUM.HOME_ADVENTURE)
end

function Activity16DungeonWindow.OnShown()
  if uis then
    Activity16DungeonWindow.UpdateInfo()
  end
end

function Activity16DungeonWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity16DungeonWindow.name)
  uis = nil
  if effect then
...
function Activity16DungeonWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_CROSS_DAY then
    ActivityDungeonMgr.CheckActivityEnd()
  end
end

return Activity16DungeonWindow
