...
function Activity9SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity9SignWindow.package, WinResConfig.Activity9SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1008_SignWindowUis(contentPane)
    Activity9SignWindow.UpdateInfo()
    Activity9SignWindow.InitBtn()
  end)
end

function Activity9SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity9SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity9SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity9SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity9SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity9SignWindow.GetRewardData(groupId)
...
function Activity9SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity9SignWindow.CloseWindow)
end

function Activity9SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity9SignWindow.name)
end

function Activity9SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity9SignWindow
