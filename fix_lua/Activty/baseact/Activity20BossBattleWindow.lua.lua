...
function Activity20BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity20BossBattleWindow.package, WinResConfig.Activity20BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1020_BossBattleWindowUis(contentPane)
    Activity20BossBattleWindow.UpdateInfo()
    Activity20BossBattleWindow.InitBtn()
  end)
end

function Activity20BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity20BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity20BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity20BossBattleWindow.FormatRemainTime(time)
...
function Activity20BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity20BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity20BossBattleWindow.CloseWindow)
end

function Activity20BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity20BossBattleWindow.name)
end

function Activity20BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity20BossBattleWindow
