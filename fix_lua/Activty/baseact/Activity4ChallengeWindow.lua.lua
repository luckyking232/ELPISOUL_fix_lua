...
function Activity4ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity4ChallengeWindow.package, WinResConfig.Activity4ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1003_ChallengeWindowUis(contentPane)
...
function Activity4ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity4ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity4ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity4ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity4ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity4ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity4ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity4ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity4ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity4ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity4ChallengeWindow.ShopInfo()
...
function Activity4ChallengeWindow.GetChapterPassById(id)
...
function Activity4ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity4ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity4ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity4ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity4ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity4ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(1668))
end

function Activity4ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity4ChallengeWindow.name)
end

function Activity4ChallengeWindow.OnShown()
  if uis then
    Activity4ChallengeWindow.UpdateInfo()
  end
end

function Activity4ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity4ChallengeWindow
