...
function Activity20ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity20ShopWindow.package, WinResConfig.Activity20ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1020_ShopWindowUis(contentPane)
    Activity20ShopWindow.UpdateInfo()
    Activity20ShopWindow.InitBtn()
  end)
end

function Activity20ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity20ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity20ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity20ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity20ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity20ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity20ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity20ShopWindow.GetShopGrid(groupId)
...
function Activity20ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity20ShopWindow.CloseWindow)
end

function Activity20ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity20ShopWindow.name)
end

function Activity20ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity20ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity20ShopWindow.UpdateInfo(true)
  end
end

return Activity20ShopWindow
