...
function Activity11BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity11BuyLevelDesWindow.package, WinResConfig.Activity11BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1011_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity11BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity11BuyLevelDesWindow.ShowAssetsTips()
...
function Activity11BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity11BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity11BuyLevelDesWindow.GetGold(buyNum)
...
function Activity11BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity11BuyLevelDesWindow.ShowReward()
  local data = Activity11BuyLevelDesWindow.GetRewardItem()
...
function Activity11BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity11BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity11BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity11BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity11BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity11BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity11PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity11BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity11PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity11BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity11BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity11BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity11BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity11BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity11BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity11BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity11BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity11BuyLevelDesWindow
