...
function Activity13BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity13BossBattleWindow.package, WinResConfig.Activity13BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1013_BossBattleWindowUis(contentPane)
    Activity13BossBattleWindow.UpdateInfo()
    Activity13BossBattleWindow.InitBtn()
  end)
end

function Activity13BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity13BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity13BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity13BossBattleWindow.FormatRemainTime(time)
...
function Activity13BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity13BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity13BossBattleWindow.CloseWindow)
end

function Activity13BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity13BossBattleWindow.name)
end

function Activity13BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity13BossBattleWindow
