...
function Activity20BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity20BuyLevelDesWindow.package, WinResConfig.Activity20BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1020_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity20BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity20BuyLevelDesWindow.ShowAssetsTips()
...
function Activity20BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity20BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity20BuyLevelDesWindow.GetGold(buyNum)
...
function Activity20BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity20BuyLevelDesWindow.ShowReward()
  local data = Activity20BuyLevelDesWindow.GetRewardItem()
...
function Activity20BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity20BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity20BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity20BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity20BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity20BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity20PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity20BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity20PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity20BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity20BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity20BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity20BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity20BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity20BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity20BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity20BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity20BuyLevelDesWindow
