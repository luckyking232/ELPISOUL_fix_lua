...
function Activity21SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity21SignWindow.package, WinResConfig.Activity21SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1021_SignWindowUis(contentPane)
    Activity21SignWindow.UpdateInfo()
    Activity21SignWindow.InitBtn()
  end)
end

function Activity21SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity21SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity21SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity21SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity21SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity21SignWindow.GetRewardData(groupId)
...
function Activity21SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity21SignWindow.CloseWindow)
end

function Activity21SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity21SignWindow.name)
end

function Activity21SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity21SignWindow
