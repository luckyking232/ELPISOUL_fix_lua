...
function Activity13TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity13TaskWindow.package, WinResConfig.Activity13TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1013_TaskWindowUis(contentPane)
...
function Activity13TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity13TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity13TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity13TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity13TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity13TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity13TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity13TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(1980)
  title.TitleTxt.text = T(1979)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity13TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity13TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity13TaskWindow.name)
end

function Activity13TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity13TaskWindow
