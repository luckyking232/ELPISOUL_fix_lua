...
function Activity5SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity5SignWindow.package, WinResConfig.Activity5SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1004_SignWindowUis(contentPane)
    Activity5SignWindow.UpdateInfo()
    Activity5SignWindow.InitBtn()
  end)
end

function Activity5SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity5SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity5SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity5SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity5SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity5SignWindow.GetRewardData(groupId)
...
function Activity5SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity5SignWindow.CloseWindow)
end

function Activity5SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity5SignWindow.name)
end

function Activity5SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity5SignWindow
