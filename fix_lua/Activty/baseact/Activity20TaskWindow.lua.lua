...
function Activity20TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity20TaskWindow.package, WinResConfig.Activity20TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1020_TaskWindowUis(contentPane)
...
function Activity20TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity20TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity20TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask(ActivityDungeonData.GetShowId()) and true or false
  end
end

function Activity20TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity20TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity20TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity20TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity20TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(2142)
  title.TitleTxt.text = T(2141)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity20TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity20TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity20TaskWindow.name)
end

function Activity20TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity20TaskWindow
