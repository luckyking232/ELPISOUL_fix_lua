...
function Activity5BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity5BuyLevelDesWindow.package, WinResConfig.Activity5BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1004_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity5BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity5BuyLevelDesWindow.ShowAssetsTips()
...
function Activity5BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity5BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity5BuyLevelDesWindow.GetGold(buyNum)
...
function Activity5BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity5BuyLevelDesWindow.ShowReward()
  local data = Activity5BuyLevelDesWindow.GetRewardItem()
...
function Activity5BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity5BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity5BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity5BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity5BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity5BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity5PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity5BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity5PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity5BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity5BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity5BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity5BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity5BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity5BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity5BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity5BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity5BuyLevelDesWindow
