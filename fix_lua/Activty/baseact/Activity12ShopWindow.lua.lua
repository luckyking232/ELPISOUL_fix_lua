...
function Activity12ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity12ShopWindow.package, WinResConfig.Activity12ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1012_ShopWindowUis(contentPane)
    Activity12ShopWindow.UpdateInfo()
    Activity12ShopWindow.InitBtn()
  end)
end

function Activity12ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity12ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity12ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity12ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity12ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity12ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity12ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity12ShopWindow.GetShopGrid(groupId)
...
function Activity12ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity12ShopWindow.CloseWindow)
end

function Activity12ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity12ShopWindow.name)
end

function Activity12ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity12ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity12ShopWindow.UpdateInfo(true)
  end
end

return Activity12ShopWindow
