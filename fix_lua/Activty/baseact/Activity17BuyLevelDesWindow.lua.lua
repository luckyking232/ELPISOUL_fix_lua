...
function Activity17BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity17BuyLevelDesWindow.package, WinResConfig.Activity17BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1017_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity17BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity17BuyLevelDesWindow.ShowAssetsTips()
...
function Activity17BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity17BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity17BuyLevelDesWindow.GetGold(buyNum)
...
function Activity17BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity17BuyLevelDesWindow.ShowReward()
  local data = Activity17BuyLevelDesWindow.GetRewardItem()
...
function Activity17BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity17BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity17BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity17BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity17BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity17BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity17PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity17BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity17PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity17BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity17BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity17BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity17BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity17BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity17BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity17BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity17BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity17BuyLevelDesWindow
