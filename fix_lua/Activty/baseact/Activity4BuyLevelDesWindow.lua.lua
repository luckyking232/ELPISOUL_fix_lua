...
function Activity4BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity4BuyLevelDesWindow.package, WinResConfig.Activity4BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1003_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity4BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity4BuyLevelDesWindow.ShowAssetsTips()
...
function Activity4BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity4BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity4BuyLevelDesWindow.GetGold(buyNum)
...
function Activity4BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity4BuyLevelDesWindow.ShowReward()
  local data = Activity4BuyLevelDesWindow.GetRewardItem()
...
function Activity4BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity4BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity4BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity4BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity4BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity4BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity4PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity4BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity4PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity4BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity4BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity4BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity4BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity4BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity4BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity4BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity4BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity4BuyLevelDesWindow
