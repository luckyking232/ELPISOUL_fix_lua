...
function Activity21PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity21PassportWindow.package, WinResConfig.Activity21PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1021_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity21PassportWindow.name, {true})
      if passInfo then
        Activity21PassportWindow.InitPassport()
        Activity21PassportWindow.CheckItemTime()
      end
...
function Activity21PassportWindow.UpdateTextDisplay()
...
function Activity21PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity21PassportWindow.name,
        com = item,
...
function Activity21PassportWindow.InitEffect()
...
function Activity21PassportWindow.InitLv()
...
function Activity21PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity21PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity21PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity21PassportWindow.SaveLevel()
  local tb = {}
...
function Activity21PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity21PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity21PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity21PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity21PassportWindow.ShowReward()
  Activity21PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity21PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity21PassportWindow.ShowLevelUp()
end

function Activity21PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity21PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity21PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity21PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity21PassportWindow.RefreshRewardEnd(lv1, Activity21PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity21PassportWindow.RefreshRewardEnd(lv2, Activity21PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity21PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity21PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity21PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity21PassportWindow.name)
...
function Activity21PassportWindow.RefreshUI()
  Activity21PassportWindow.GetExpMxp()
  Activity21PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity21PassportWindow.GetExpMxp()
...
function Activity21PassportWindow.SetLevelInfo()
...
function Activity21PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity21PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity21PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity21PassportWindow.SetListPos()
  local list = uis.Main.RewardRegion.MiddleList.MiddleList
  local len = list.numItems
  for i = 1, len do
    if passInfo.lv == i - 3 or len - 3 <= passInfo.lv and i == passInfo.lv then
      list:ScrollToView(i - 1)
      Activity21PassportWindow.RefreshListReward(list.scrollPane.posX)
      break
    end
  end
...
function Activity21PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity21PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity21PassportWindow.RefreshRewardShow(data, objReward)
      Activity21PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity21PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity21PassportWindow.ShowGetEffect()
  local minScale = 0.82
  for i = 1, 2 do
    if animTween[i] then
...
function Activity21PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity21PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity21PassportWindow.IsReward(passId, lv)
...
function Activity21PassportWindow.RefreshRewardFirst()
  Activity21PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity21PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity21PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity21PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity21PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity21PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity21PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity21PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity21PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity21PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity21PassportWindow.RefreshRewardShow(data, Item)
...
function Activity21PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity21PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity21PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity21PassportWindow.name)
end

function Activity21PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity21PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity21PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity21PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity21BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity21PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity21PassportWindow.InitTask()
...
function Activity21PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity21PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity21PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity21PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity21PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity21PassportWindow.name) then
        local lvUp = Activity21PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity21PassportWindow.ShowExpBarAnim(para)
    else
      Activity21PassportWindow.SetLevelInfo()
    end
...
function Activity21PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity21PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity21PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity21PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity21PassportWindow
