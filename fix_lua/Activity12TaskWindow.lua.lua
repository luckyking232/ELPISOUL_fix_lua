...
function Activity12TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity12TaskWindow.package, WinResConfig.Activity12TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1012_TaskWindowUis(contentPane)
...
function Activity12TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity12TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity12TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity12TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity12TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity12TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity12TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity12TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(1933)
  title.TitleTxt.text = T(1932)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity12TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity12TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity12TaskWindow.name)
end

function Activity12TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity12TaskWindow
