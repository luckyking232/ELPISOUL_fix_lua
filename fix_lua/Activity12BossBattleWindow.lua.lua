...
function Activity12BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity12BossBattleWindow.package, WinResConfig.Activity12BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1012_BossBattleWindowUis(contentPane)
    Activity12BossBattleWindow.UpdateInfo()
    Activity12BossBattleWindow.InitBtn()
  end)
end

function Activity12BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity12BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity12BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity12BossBattleWindow.FormatRemainTime(time)
...
function Activity12BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity12BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity12BossBattleWindow.CloseWindow)
end

function Activity12BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity12BossBattleWindow.name)
end

function Activity12BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity12BossBattleWindow
