    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity3_MiniGameService.MiniGameInfoReq(RefreshRewardList)
          end
...
function Activity3MiniGameTaskWindow.ReInitData()
end

function Activity3MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity3MiniGameTaskWindow.package, WinResConfig.Activity3MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1002_MiniTaskWindowUis(contentPane)
    Activity3MiniGameTaskWindow.UpdateInfo()
    Activity3MiniGameTaskWindow.InitBtn()
  end)
end

function Activity3MiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity3_MiniGameData.GetMiniGameInfo()
...
function Activity3MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity3MiniGameTaskWindow.name)
  end)
end

function Activity3MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity3MiniGameTaskWindow
