...
function Activity8PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity8PassportWindow.package, WinResConfig.Activity8PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1007_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity8PassportWindow.name, {true})
      if passInfo then
        Activity8PassportWindow.InitPassport()
        Activity8PassportWindow.CheckItemTime()
      end
...
function Activity8PassportWindow.UpdateTextDisplay()
...
function Activity8PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity8PassportWindow.name,
        com = item,
...
function Activity8PassportWindow.InitEffect()
...
function Activity8PassportWindow.InitLv()
...
function Activity8PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity8PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity8PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity8PassportWindow.SaveLevel()
  local tb = {}
...
function Activity8PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity8PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity8PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity8PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity8PassportWindow.ShowReward()
  Activity8PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity8PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity8PassportWindow.ShowLevelUp()
end

function Activity8PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity8PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity8PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity8PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity8PassportWindow.RefreshRewardEnd(lv1, Activity8PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity8PassportWindow.RefreshRewardEnd(lv2, Activity8PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity8PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity8PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity8PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity8PassportWindow.name)
...
function Activity8PassportWindow.RefreshUI()
  Activity8PassportWindow.GetExpMxp()
  Activity8PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity8PassportWindow.GetExpMxp()
...
function Activity8PassportWindow.SetLevelInfo()
...
function Activity8PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity8PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity8PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity8PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity8PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity8PassportWindow.RefreshRewardShow(data, objReward)
      Activity8PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity8PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity8PassportWindow.ShowGetEffect()
  local minScale = 0.82
  for i = 1, 2 do
    if animTween[i] then
      ...
      for k, v in ipairs(animTween[i]) do
        TimerUtil.setTimeout(0.2 + k * 0.1, function()
          if 1 == v.type then
            UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/PassCard/FX_ui_passcard_007_light_up.prefab", v.reward.parent:GetChild("Effect1Holder"), nil, true, function()
            ...
            UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/PassCard/FX_ui_passcard_007_light_down.prefab", v.reward.parent:GetChild("Effect2Holder"), nil, true, function()
              UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/PassCard/FX_passcard_get_light.prefab", v.reward:GetChild("EffectHolder"))
              animTween[i][k] = nil
            end)
          end
...
function Activity8PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity8PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity8PassportWindow.IsReward(passId, lv)
...
function Activity8PassportWindow.RefreshRewardFirst()
  Activity8PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity8PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity8PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity8PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity8PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity8PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity8PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity8PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity8PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity8PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity8PassportWindow.RefreshRewardShow(data, Item)
...
function Activity8PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity8PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity8PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity8PassportWindow.name)
end

function Activity8PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity8PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity8PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity8PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity8BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity8PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity8PassportWindow.InitTask()
...
function Activity8PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity8PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity8PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity8PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity8PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity8PassportWindow.name) then
        local lvUp = Activity8PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity8PassportWindow.ShowExpBarAnim(para)
    else
      Activity8PassportWindow.SetLevelInfo()
    end
...
function Activity8PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity8PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity8PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity8PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity8PassportWindow
