...
function Activity11SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity11SignWindow.package, WinResConfig.Activity11SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1011_SignWindowUis(contentPane)
    Activity11SignWindow.UpdateInfo()
    Activity11SignWindow.InitBtn()
  end)
end

function Activity11SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity11SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity11SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity11SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity11SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity11SignWindow.GetRewardData(groupId)
...
function Activity11SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity11SignWindow.CloseWindow)
end

function Activity11SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity11SignWindow.name)
end

function Activity11SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity11SignWindow
