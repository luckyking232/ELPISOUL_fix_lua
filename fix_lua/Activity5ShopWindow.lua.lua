...
function Activity5ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity5ShopWindow.package, WinResConfig.Activity5ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1004_ShopWindowUis(contentPane)
    Activity5ShopWindow.UpdateInfo()
    Activity5ShopWindow.InitBtn()
  end)
end

function Activity5ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity5ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity5ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity5ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity5ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity5ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity5ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity5ShopWindow.GetShopGrid(groupId)
...
function Activity5ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity5ShopWindow.CloseWindow)
end

function Activity5ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity5ShopWindow.name)
end

function Activity5ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity5ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity5ShopWindow.UpdateInfo(true)
  end
end

return Activity5ShopWindow
