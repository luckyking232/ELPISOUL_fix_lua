...
function Activity8ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity8ShopWindow.package, WinResConfig.Activity8ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1007_ShopWindowUis(contentPane)
    Activity8ShopWindow.UpdateInfo()
    Activity8ShopWindow.InitBtn()
  end)
end

function Activity8ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity8ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity8ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity8ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity8ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity8ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity8ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity8ShopWindow.GetShopGrid(groupId)
...
function Activity8ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity8ShopWindow.CloseWindow)
end

function Activity8ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity8ShopWindow.name)
end

function Activity8ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity8ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity8ShopWindow.UpdateInfo(true)
  end
end

return Activity8ShopWindow
