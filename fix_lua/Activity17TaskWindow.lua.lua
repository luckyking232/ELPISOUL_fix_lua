...
function Activity17TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity17TaskWindow.package, WinResConfig.Activity17TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1017_TaskWindowUis(contentPane)
...
function Activity17TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity17TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity17TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask(ActivityDungeonData.GetShowId()) and true or false
  end
end

function Activity17TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity17TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity17TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity17TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity17TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(2087)
  title.TitleTxt.text = T(2086)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity17TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity17TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity17TaskWindow.name)
end

function Activity17TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity17TaskWindow
