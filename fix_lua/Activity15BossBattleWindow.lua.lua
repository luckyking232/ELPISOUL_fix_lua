...
function Activity15BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity15BossBattleWindow.package, WinResConfig.Activity15BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1015_BossBattleWindowUis(contentPane)
    Activity15BossBattleWindow.UpdateInfo()
    Activity15BossBattleWindow.InitBtn()
  end)
end

function Activity15BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity15BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity15BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity15BossBattleWindow.FormatRemainTime(time)
...
function Activity15BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity15BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity15BossBattleWindow.CloseWindow)
end

function Activity15BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity15BossBattleWindow.name)
end

function Activity15BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity15BossBattleWindow
