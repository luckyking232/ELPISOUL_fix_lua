...
function Activity9ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity9ShopWindow.package, WinResConfig.Activity9ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1008_ShopWindowUis(contentPane)
    Activity9ShopWindow.UpdateInfo()
    Activity9ShopWindow.InitBtn()
  end)
end

function Activity9ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity9ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity9ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity9ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity9ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity9ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity9ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity9ShopWindow.GetShopGrid(groupId)
...
function Activity9ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity9ShopWindow.CloseWindow)
end

function Activity9ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity9ShopWindow.name)
end

function Activity9ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity9ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity9ShopWindow.UpdateInfo(true)
  end
end

return Activity9ShopWindow
