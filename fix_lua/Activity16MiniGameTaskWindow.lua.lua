    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity16_MiniGameService.MiniGameInfoReq(RefreshRewardList)
          end
...
function Activity16MiniGameTaskWindow.ReInitData()
end

function Activity16MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity16MiniGameTaskWindow.package, WinResConfig.Activity16MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1016_MiniTaskWindowUis(contentPane)
    Activity16MiniGameTaskWindow.UpdateInfo()
    Activity16MiniGameTaskWindow.InitBtn()
  end)
end

function Activity16MiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity16_MiniGameData.GetMiniGameInfo()
...
function Activity16MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity16MiniGameTaskWindow.name)
  end)
end

function Activity16MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity16MiniGameTaskWindow
