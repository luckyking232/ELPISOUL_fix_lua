...
function Activity17BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity17BossBattleWindow.package, WinResConfig.Activity17BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1017_BossBattleWindowUis(contentPane)
    Activity17BossBattleWindow.UpdateInfo()
    Activity17BossBattleWindow.InitBtn()
  end)
end

function Activity17BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity17BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity17BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity17BossBattleWindow.FormatRemainTime(time)
...
function Activity17BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity17BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity17BossBattleWindow.CloseWindow)
end

function Activity17BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity17BossBattleWindow.name)
end

function Activity17BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity17BossBattleWindow
