...
function Activity9BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity9BossBattleWindow.package, WinResConfig.Activity9BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1008_BossBattleWindowUis(contentPane)
    Activity9BossBattleWindow.UpdateInfo()
    Activity9BossBattleWindow.InitBtn()
  end)
end

function Activity9BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity9BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity9BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity9BossBattleWindow.FormatRemainTime(time)
...
function Activity9BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity9BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity9BossBattleWindow.CloseWindow)
end

function Activity9BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity9BossBattleWindow.name)
end

function Activity9BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity9BossBattleWindow
