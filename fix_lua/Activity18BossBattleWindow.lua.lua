...
function Activity18BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity18BossBattleWindow.package, WinResConfig.Activity18BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1018_BossBattleWindowUis(contentPane)
    Activity18BossBattleWindow.UpdateInfo()
    Activity18BossBattleWindow.InitBtn()
  end)
end

function Activity18BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity18BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity18BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity18BossBattleWindow.FormatRemainTime(time)
...
function Activity18BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity18BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity18BossBattleWindow.CloseWindow)
end

function Activity18BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity18BossBattleWindow.name)
end

function Activity18BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity18BossBattleWindow
