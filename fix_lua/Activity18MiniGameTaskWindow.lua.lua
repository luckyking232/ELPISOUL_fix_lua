    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity18_MiniGameService.MiniGameInfoReq(RefreshRewardList)
          end
...
function Activity18MiniGameTaskWindow.ReInitData()
end

function Activity18MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity18MiniGameTaskWindow.package, WinResConfig.Activity18MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1018_MiniTaskWindowUis(contentPane)
    Activity18MiniGameTaskWindow.UpdateInfo()
    Activity18MiniGameTaskWindow.InitBtn()
  end)
end

function Activity18MiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity18_MiniGameData.GetMiniGameInfo()
...
function Activity18MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity18MiniGameTaskWindow.name)
  end)
end

function Activity18MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity18MiniGameTaskWindow
