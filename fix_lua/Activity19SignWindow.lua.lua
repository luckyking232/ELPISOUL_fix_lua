...
function Activity19SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity19SignWindow.package, WinResConfig.Activity19SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1019_SignWindowUis(contentPane)
    Activity19SignWindow.UpdateInfo()
    Activity19SignWindow.InitBtn()
  end)
end

function Activity19SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity19SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity19SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity19SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity19SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity19SignWindow.GetRewardData(groupId)
...
function Activity19SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity19SignWindow.CloseWindow)
end

function Activity19SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity19SignWindow.name)
end

function Activity19SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity19SignWindow
