    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity19_MiniGameService.MiniGameInfoReq(RefreshRewardList)
          end
...
function Activity19MiniGameTaskWindow.ReInitData()
end

function Activity19MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity19MiniGameTaskWindow.package, WinResConfig.Activity19MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1019_MiniTaskWindowUis(contentPane)
    Activity19MiniGameTaskWindow.UpdateInfo()
    Activity19MiniGameTaskWindow.InitBtn()
  end)
end

function Activity19MiniGameTaskWindow.UpdateInfo()
  local info = Activity19_MiniGameData.GetMiniGameInfo(gameId)
...
function Activity19MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity19MiniGameTaskWindow.name)
  end)
end

function Activity19MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity19MiniGameTaskWindow
