...
function Activity22BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity22BossBattleWindow.package, WinResConfig.Activity22BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1022_BossBattleWindowUis(contentPane)
    Activity22BossBattleWindow.UpdateInfo()
    Activity22BossBattleWindow.InitBtn()
  end)
end

function Activity22BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity22BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity22BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity22BossBattleWindow.FormatRemainTime(time)
...
function Activity22BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity22BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity22BossBattleWindow.CloseWindow)
end

function Activity22BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity22BossBattleWindow.name)
end

function Activity22BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity22BossBattleWindow
