    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity22_MiniGameService.MiniGameInfoReq(RefreshRewardList)
          end
...
function Activity22MiniGameTaskWindow.ReInitData()
end

function Activity22MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity22MiniGameTaskWindow.package, WinResConfig.Activity22MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1022_MiniTaskWindowUis(contentPane)
    Activity22MiniGameTaskWindow.UpdateInfo()
    Activity22MiniGameTaskWindow.InitBtn()
  end)
end

function Activity22MiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity22_MiniGameData.GetMiniGameInfo()
...
function Activity22MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity22MiniGameTaskWindow.name)
  end)
end

function Activity22MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity22MiniGameTaskWindow
