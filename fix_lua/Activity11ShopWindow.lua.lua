...
function Activity11ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity11ShopWindow.package, WinResConfig.Activity11ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1011_ShopWindowUis(contentPane)
    Activity11ShopWindow.UpdateInfo()
    Activity11ShopWindow.InitBtn()
  end)
end

function Activity11ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity11ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity11ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity11ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity11ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity11ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity11ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity11ShopWindow.GetShopGrid(groupId)
...
function Activity11ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity11ShopWindow.CloseWindow)
end

function Activity11ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity11ShopWindow.name)
end

function Activity11ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity11ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity11ShopWindow.UpdateInfo(true)
  end
end

return Activity11ShopWindow
