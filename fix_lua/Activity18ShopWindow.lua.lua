...
function Activity18ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity18ShopWindow.package, WinResConfig.Activity18ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1018_ShopWindowUis(contentPane)
    Activity18ShopWindow.UpdateInfo()
    Activity18ShopWindow.InitBtn()
  end)
end

function Activity18ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity18ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity18ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity18ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity18ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity18ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity18ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity18ShopWindow.GetShopGrid(groupId)
...
function Activity18ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity18ShopWindow.CloseWindow)
end

function Activity18ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity18ShopWindow.name)
end

function Activity18ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity18ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity18ShopWindow.UpdateInfo(true)
  end
end

return Activity18ShopWindow
