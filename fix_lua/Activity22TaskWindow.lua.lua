...
function Activity22TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity22TaskWindow.package, WinResConfig.Activity22TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1022_TaskWindowUis(contentPane)
...
function Activity22TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity22TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity22TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask(ActivityDungeonData.GetShowId()) and true or false
  end
end

function Activity22TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity22TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity22TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity22TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity22TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(2188)
  title.TitleTxt.text = T(2187)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity22TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity22TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity22TaskWindow.name)
end

function Activity22TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity22TaskWindow
