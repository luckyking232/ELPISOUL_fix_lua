...
function Activity18SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity18SignWindow.package, WinResConfig.Activity18SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1018_SignWindowUis(contentPane)
    Activity18SignWindow.UpdateInfo()
    Activity18SignWindow.InitBtn()
  end)
end

function Activity18SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity18SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity18SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity18SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity18SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity18SignWindow.GetRewardData(groupId)
...
function Activity18SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity18SignWindow.CloseWindow)
end

function Activity18SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity18SignWindow.name)
end

function Activity18SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity18SignWindow
