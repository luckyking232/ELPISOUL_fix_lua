...
function Activity8BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity8BossBattleWindow.package, WinResConfig.Activity8BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1007_BossBattleWindowUis(contentPane)
    Activity8BossBattleWindow.UpdateInfo()
    Activity8BossBattleWindow.InitBtn()
  end)
end

function Activity8BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity8BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity8BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity8BossBattleWindow.FormatRemainTime(time)
...
function Activity8BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity8BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity8BossBattleWindow.CloseWindow)
end

function Activity8BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity8BossBattleWindow.name)
end

function Activity8BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity8BossBattleWindow
