...
function Activity19ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity19ChallengeWindow.package, WinResConfig.Activity19ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1019_ChallengeWindowUis(contentPane)
...
function Activity19ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity19ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity19ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity19ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity19ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity19ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity19ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity19ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity19ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity19ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity19ChallengeWindow.ShopInfo()
...
function Activity19ChallengeWindow.GetChapterPassById(id)
...
function Activity19ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity19ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity19ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity19ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity19ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity19ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(2125))
end

function Activity19ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity19ChallengeWindow.name)
end

function Activity19ChallengeWindow.OnShown()
  if uis then
    Activity19ChallengeWindow.UpdateInfo()
  end
end

function Activity19ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity19ChallengeWindow
