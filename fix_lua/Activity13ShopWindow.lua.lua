...
function Activity13ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity13ShopWindow.package, WinResConfig.Activity13ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1013_ShopWindowUis(contentPane)
    Activity13ShopWindow.UpdateInfo()
    Activity13ShopWindow.InitBtn()
  end)
end

function Activity13ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity13ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity13ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity13ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity13ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity13ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity13ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity13ShopWindow.GetShopGrid(groupId)
...
function Activity13ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity13ShopWindow.CloseWindow)
end

function Activity13ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity13ShopWindow.name)
end

function Activity13ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity13ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity13ShopWindow.UpdateInfo(true)
  end
end

return Activity13ShopWindow
