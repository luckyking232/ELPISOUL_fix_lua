    ...
    Activity18_MiniGameService.MiniGameInfoReq(function()
      RedDotMgr.AddNode({
        ...
        visibleFunc = function()
          return RedDotActivityDungeon.MiniGame18_TaskRewardable() or RedDotActivityDungeon.MiniGame18_DailyTaskRewardable()
        end,
        dataType = RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON
      })
      RedDotMgr.UpdateNodeByWindowName(WinResConfig.Activity18DungeonWindow.name)
    end)
  end
end

function Activity18DungeonWindow.ReInitData()
end

function Activity18DungeonWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity18DungeonWindow.package, WinResConfig.Activity18DungeonWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1018_ActivityDungeonWindowUis(contentPane)
    ...
    if plotShowId then
      ...
      if reviewWord then
        reviewWord.onClick:Set(function()
          ...
          if id then
            OpenWindow(WinResConfig.Activity18PlotWindow.name, nil, id, configData)
          end
        end)
...
function Activity18DungeonWindow.LoadBg()
...
function Activity18DungeonWindow.InitRedDot()
  RedDotMgr.AddNode({
    windowName = WinResConfig.Activity18DungeonWindow.name,
...
function Activity18DungeonWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
    ...
    if configData.boss_chapter_ids then
      ...
      if 2 == #arr then
        ...
        uis.Main.BossBtn.onClick:Set(function()
          if lock then
            OpenWindow(WinResConfig.Activity18BossBattleWindow.name)
          else
            FloatTipsUtil.ShowWarnTips(tips)
          end
        end)
...
function Activity18DungeonWindow.InitBtnTxt()
  UIUtil.SetText(uis.Main.BossBtn, T(2110))
  UIUtil.SetText(uis.Main.MaterialBtn, T(2111))
  uis.Main.NormalBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity18ChallengeWindow.name)
  end)
  UIUtil.SetText(uis.Main.NormalBtn, T(2112))
    ...
    if passData then
      uis.Main.PassBtn.onClick:Set(function()
        PassportService.GetBattlePassInfoReq(function()
          ...
          OpenWindow(WinResConfig.Activity18PassportWindow.name)
        end)
      end)
  ...
  UIUtil.SetText(uis.Main.PlotBtn, T(2107))
  uis.Main.ShopBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity18ShopWindow.name)
  end)
  UIUtil.SetText(uis.Main.ShopBtn, T(2109))
  uis.Main.SignBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity18SignWindow.name)
  end)
  UIUtil.SetText(uis.Main.SignBtn, T(2113))
  uis.Main.TaskBtn.onClick:Set(function()
    ActivityDungeonService.GetActivityAllReq(function()
      OpenWindow(WinResConfig.Activity18TaskWindow.name)
    end)
  end)
  UIUtil.SetText(uis.Main.TaskBtn, T(2108))
  uis.Main.MiniGameBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity18MiniGameMainWindow.name)
  end)
end

function Activity18DungeonWindow.InitBtn()
  jumpTb = CurrencyReturnWindow.SetCurrencyReturn(WinResConfig.Activity18DungeonWindow.name, uis.Main.CurrencyReturn, FEATURE_ENUM.HOME_ADVENTURE)
end

function Activity18DungeonWindow.OnShown()
  if uis then
    Activity18DungeonWindow.UpdateInfo()
  end
end

function Activity18DungeonWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity18DungeonWindow.name)
  uis = nil
  if effect then
...
function Activity18DungeonWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_CROSS_DAY then
    ActivityDungeonMgr.CheckActivityEnd()
  end
end

return Activity18DungeonWindow
