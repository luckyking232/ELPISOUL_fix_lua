...
function Activity6TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity6TaskWindow.package, WinResConfig.Activity6TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1005_TaskWindowUis(contentPane)
...
function Activity6TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity6TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity6TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity6TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity6TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity6TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity6TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity6TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(1770)
  title.TitleTxt.text = T(1769)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity6TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity6TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity6TaskWindow.name)
end

function Activity6TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity6TaskWindow
