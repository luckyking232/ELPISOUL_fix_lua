...
function Activity15BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity15BuyLevelDesWindow.package, WinResConfig.Activity15BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1015_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity15BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity15BuyLevelDesWindow.ShowAssetsTips()
...
function Activity15BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity15BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity15BuyLevelDesWindow.GetGold(buyNum)
...
function Activity15BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity15BuyLevelDesWindow.ShowReward()
  local data = Activity15BuyLevelDesWindow.GetRewardItem()
...
function Activity15BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity15BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity15BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity15BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity15BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity15BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity15PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity15BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity15PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity15BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity15BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity15BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity15BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity15BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity15BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity15BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity15BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity15BuyLevelDesWindow
