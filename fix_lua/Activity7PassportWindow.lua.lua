...
function Activity7PassportWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity7PassportWindow.package, WinResConfig.Activity7PassportWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1006_PassportWindowUis(contentPane)
    ...
    if bridgeObj.argTable[1] then
      ...
      UIMgr:SetWindowArgs(WinResConfig.Activity7PassportWindow.name, {true})
      if passInfo then
        Activity7PassportWindow.InitPassport()
        Activity7PassportWindow.CheckItemTime()
      end
...
function Activity7PassportWindow.UpdateTextDisplay()
...
function Activity7PassportWindow.InitPassport()
  ...
  function uis.Main.TabRegion.TabList.itemRenderer(i, item)
    ...
    if 0 == i then
      RedDotMgr.AddNode({
        windowName = WinResConfig.Activity7PassportWindow.name,
        com = item,
...
function Activity7PassportWindow.InitEffect()
...
function Activity7PassportWindow.InitLv()
...
function Activity7PassportWindow.ShowLevelUp()
  local lv = passLv[passInfo.passPortId]
  if lv and lv < passInfo.lv then
    passLv[passInfo.passPortId] = passInfo.lv
    Activity7PassportWindow.SaveLevel()
    OpenWindow(WinResConfig.Activity7PassportLevelUpTipsWindow.name, nil, passInfo.lv)
    return true
  end
end

function Activity7PassportWindow.SaveLevel()
  local tb = {}
...
function Activity7PassportWindow.GetRewardByLv(tb, lv)
  if lv and tb then
    for i, v in pairs(tb) do
      if v.level == lv then
        return v
...
function Activity7PassportWindow.GetRewardNum(ids)
  local rewardNum = 0
...
function Activity7PassportWindow.IsGetAllReward(data)
  if data then
    ...
    local num = Activity7PassportWindow.GetRewardNum(data.openList)
    return len >= num
  end
end

function Activity7PassportWindow.ShowReward()
  Activity7PassportWindow.RefreshUI()
  uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.onScroll:Add(function()
    Activity7PassportWindow.RefreshListReward(uis.Main.RewardRegion.MiddleList.MiddleList.scrollPane.posX)
  end)
  Activity7PassportWindow.ShowLevelUp()
end

function Activity7PassportWindow.GetUpdateLv(bar, tb)
  for i = 1, #tb do
    if bar <= tb[i] then
      return tb[i]
    end
...
function Activity7PassportWindow.RefreshListReward(posX)
  local curLv = math.floor(posX / 120) + listPage
  if lvMax and curLv > lvMax then
    curLv = lvMax
  end
  local lv2 = Activity7PassportWindow.GetUpdateLv(curLv, rewardType[1])
  local lv1 = Activity7PassportWindow.GetUpdateLv(curLv, rewardType[1])
  if lv1 ~= rewardLv[1] then
    rewardLv[1] = lv1
    Activity7PassportWindow.RefreshRewardEnd(lv1, Activity7PassportWindow.GetRewardByLv(rewardData[1], rewardLv[1]), true)
  end
  if lv2 ~= rewardLv[2] then
    rewardLv[2] = lv2
    Activity7PassportWindow.RefreshRewardEnd(lv2, Activity7PassportWindow.GetRewardByLv(rewardData[2], rewardLv[2]), false)
  end
end

function Activity7PassportWindow.CheckItemTime()
  if passInfo then
...
function Activity7PassportWindow.ShowQuitTips()
  MessageBox.Show(T(409), {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity7PassportWindow.name)
    end
  }, nil, {
    touchCallback = function()
      UIMgr:CloseWindow(WinResConfig.Activity7PassportWindow.name)
...
function Activity7PassportWindow.RefreshUI()
  Activity7PassportWindow.GetExpMxp()
  Activity7PassportWindow.SetLevelInfo()
  isShowAllGet = false
...
function Activity7PassportWindow.GetExpMxp()
...
function Activity7PassportWindow.SetLevelInfo()
...
function Activity7PassportWindow.InitList()
  ...
  function list.itemRenderer(i, reward)
    ...
    Activity7PassportWindow.RefreshReward(rewardData[1][k], reward:GetChild("Reward1"), passInfo.openList[1], 1)
    Activity7PassportWindow.RefreshReward(rewardData[2][k], reward:GetChild("Reward2"), passInfo.openList[2], 2)
  end
...
function Activity7PassportWindow.RefreshReward(data, reward, lock, type)
  if reward and data then
    ...
    if data.reward and data.reward ~= "" then
      if lock then
        state = Activity7PassportWindow.IsReward(data.pass_id, data.level)
      end
      ...
      Activity7PassportWindow.RefreshRewardShow(data, objReward)
      Activity7PassportWindow.RefreshRewardState(objReward, state)
      ChangeUIController(reward, "c2", 0)
    else
      ChangeUIController(reward, "c2", 1)
    end
    reward.onClick:Set(function()
      Activity7PassportWindow.RewardItemClick(data, state)
    end)
  end
end

function Activity7PassportWindow.ShowGetEffect()
  local minScale = 0.82
  for i = 1, 2 do
    if animTween[i] then
...
function Activity7PassportWindow.RewardItemClick(data, state)
    ...
    PassportService.BattlePassGetRewardReq(passInfo.passPortId, data.pass_id, data.level, false, function(msg)
      ...
      if msg.goods then
        if #msg.goods > 0 then
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity7PassportWindow.name, WindowMsgEnum.Passport.GET_REWARD)
        else
...
function Activity7PassportWindow.IsReward(passId, lv)
...
function Activity7PassportWindow.RefreshRewardFirst()
  Activity7PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.StartTwo.Pic.c1Ctr)
end

function Activity7PassportWindow.RefreshRewardEnd(showLv, data, page)
  if showLv then
    uis.Main.RewardRegion.EndTwo.LevelTxt.text = showLv
  end
  if data then
    local state = Activity7PassportWindow.IsReward(data.pass_id, showLv)
    if page then
      Activity7PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item1.root)
      Activity7PassportWindow.ShowState(passInfo.openList[1], uis.Main.RewardRegion.EndTwo.Item1.c1Ctr)
      if passInfo.openList[1] then
        Activity7PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item1.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
      ...
      Activity7PassportWindow.RefreshRewardShow(data, uis.Main.RewardRegion.EndTwo.Item2.root)
      Activity7PassportWindow.ShowState(passInfo.openList[2], uis.Main.RewardRegion.EndTwo.Item2.c1Ctr)
      if passInfo.openList[2] then
        Activity7PassportWindow.RefreshRewardState(uis.Main.RewardRegion.EndTwo.Item2.root, state)
        if 2 == state then
          uis.Main.RewardRegion.EndTwo.Item2.EffectHolder.visible = false
...
function Activity7PassportWindow.ShowState(bolLock, ctr)
  if bolLock then
    ctr.selectedIndex = 0
  else
    ctr.selectedIndex = 1
...
function Activity7PassportWindow.RefreshRewardShow(data, Item)
...
function Activity7PassportWindow.RefreshRewardState(Item, isReward)
  if 1 == isReward then
...
function Activity7PassportWindow.PassportActivate()
  PassportService.GetBattlePassInfoReq(function()
    passInfo = PassportData.GetActivityPassport()
  end)
end

function Activity7PassportWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity7PassportWindow.name)
end

function Activity7PassportWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity7PassportWindow.CloseWindow)
  if passInfo then
    ...
    uis.Main.AllGetBtn.onClick:Set(function()
      ...
      elseif 1 == uis.Main.c1Ctr.selectedIndex then
        PassportService.BattlePassTaskRewardReq(passInfo.passPortId, 0, function(msg)
          ...
          UIMgr:SendWindowMessage(WinResConfig.Activity7PassportWindow.name, WindowMsgEnum.Passport.GET_TASK_REWARD, msg.exp)
          RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
        end, function(code)
          if code == ProtoEnum.RET_CODE.RC_BATTLEPASS_OUT_TIME then
            Activity7PassportWindow.ShowQuitTips()
    ...
    uis.Main.Level.LevelBuyBtn.onClick:Set(function()
      if passInfo.lv < lvMax then
        OpenWindow(WinResConfig.Activity7BuyLevelDesWindow.name, nil, passInfo.passPortId)
      end
    end)
  end
end

function Activity7PassportWindow.ShowExpBarAnim(expChange)
  if expChange then
...
function Activity7PassportWindow.InitTask()
...
function Activity7PassportWindow.InitClothes()
  if passPortConfig then
...
function Activity7PassportWindow.SetLastLv()
  if nil == lastLv then
    lastLv = {}
  end
...
function Activity7PassportWindow.PlayFirstRewardEffect()
  for i = 1, 2 do
...
function Activity7PassportWindow.PlayEndRewardEffect()
  for i = 1, 2 do
...
function Activity7PassportWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Passport.BUY then
    passInfo = PassportData.GetActivityPassport()
    if 0 == uis.Main.c1Ctr.selectedIndex then
      if UIMgr:IsWindowOpen(WinResConfig.Activity7PassportWindow.name) then
        local lvUp = Activity7PassportWindow.ShowLevelUp()
        if lvUp then
          passLv[passInfo.passPortId] = passInfo.lv
          return
        end
  ...
  elseif msgId == WindowMsgEnum.Passport.GET_TASK_REWARD then
    ...
    if para and para > 0 then
      Activity7PassportWindow.ShowExpBarAnim(para)
    else
      Activity7PassportWindow.SetLevelInfo()
    end
...
function Activity7PassportWindow.OnShown()
  if uis then
    if waitMessageId then
      Activity7PassportWindow.HandleMessage(waitMessageId)
    end
    PassportService.GetBattlePassInfoReq(function()
      passInfo = PassportData.GetActivityPassport()
      if passInfo then
...
function Activity7PassportWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity7PassportWindow.name)
  uis = nil
  contentPane = nil
  lvMax = nil
  expMax = nil
...
return Activity7PassportWindow
