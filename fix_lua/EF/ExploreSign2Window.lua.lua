...
function ExploreSign2Window.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.ExploreSign2Window.package, WinResConfig.ExploreSign2Window.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetExploreSign02_ExploreSignWindowUis(contentPane)
...
function ExploreSign2Window.ShowItem()
  ...
  for i = 1, #btn do
    ...
    if data.isTodaySearch then
      if signIndex[i] then
        ExploreSign2Window.ShowItemReward(btn[i], signIndex[i])
...
function ExploreSign2Window.UpdateTextDisplay()
  UIUtil.SetBtnText(uis.Main.Explore.ExploreBtn, T(736))
end

function ExploreSign2Window.InitBtn()
  ...
  uis.Main.CloseBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.ExploreSign2Window.name)
  end)
end

function ExploreSign2Window.ShowItemReward(btn, itemTuple)
...
function ExploreSign2Window.ShowReward()
  signIndex[data.choosePos[1]] = data.dropRewards
  signIndex[data.choosePos[2]] = data.showRewards
  showIndex = data.choosePos[2]
  for i, v in pairs(signIndex) do
    ExploreSign2Window.ShowItemReward(uis.Main.Map[string.format("Place%d", i)].PlaceBtn, v)
    ChangeUIController(uis.Main.Map[string.format("Place%d", i)].PlaceBtn, "c2", 2)
  end
end

function ExploreSign2Window.ShowRewardEffect()
...
function ExploreSign2Window.OnClose()
  uis = nil
  contentPane = nil
  if data then
...
function ExploreSign2Window.CheckActivityEnd()
