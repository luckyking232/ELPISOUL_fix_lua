...
function Activity20ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity20ChallengeWindow.package, WinResConfig.Activity20ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1020_ChallengeWindowUis(contentPane)
...
function Activity20ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity20ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity20ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity20ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity20ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity20ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity20ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity20ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity20ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity20ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity20ChallengeWindow.ShopInfo()
...
function Activity20ChallengeWindow.GetChapterPassById(id)
...
function Activity20ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity20ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity20ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity20ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity20ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity20ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(2136))
end

function Activity20ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity20ChallengeWindow.name)
end

function Activity20ChallengeWindow.OnShown()
  if uis then
    Activity20ChallengeWindow.UpdateInfo()
  end
end

function Activity20ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity20ChallengeWindow
