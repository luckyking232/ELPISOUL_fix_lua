...
function Activity2BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity2BuyLevelDesWindow.package, WinResConfig.Activity2BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1001_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity2BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity2BuyLevelDesWindow.ShowAssetsTips()
...
function Activity2BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity2BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity2BuyLevelDesWindow.GetGold(buyNum)
...
function Activity2BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity2BuyLevelDesWindow.ShowReward()
  local data = Activity2BuyLevelDesWindow.GetRewardItem()
...
function Activity2BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity2BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity2BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity2BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity2BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity2BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity2PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity2BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity2PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity2BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity2BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity2BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity2BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity2BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity2BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity2BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity2BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity2BuyLevelDesWindow
