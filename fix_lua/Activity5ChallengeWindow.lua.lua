...
function Activity5ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity5ChallengeWindow.package, WinResConfig.Activity5ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1004_ChallengeWindowUis(contentPane)
...
function Activity5ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity5ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity5ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity5ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity5ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity5ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity5ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity5ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity5ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity5ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity5ChallengeWindow.ShopInfo()
...
function Activity5ChallengeWindow.GetChapterPassById(id)
...
function Activity5ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity5ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity5ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity5ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity5ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity5ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(1695))
end

function Activity5ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity5ChallengeWindow.name)
end

function Activity5ChallengeWindow.OnShown()
  if uis then
    Activity5ChallengeWindow.UpdateInfo()
  end
end

function Activity5ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity5ChallengeWindow
