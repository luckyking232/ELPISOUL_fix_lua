...
function Activity11BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity11BossBattleWindow.package, WinResConfig.Activity11BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1011_BossBattleWindowUis(contentPane)
    Activity11BossBattleWindow.UpdateInfo()
    Activity11BossBattleWindow.InitBtn()
  end)
end

function Activity11BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity11BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity11BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity11BossBattleWindow.FormatRemainTime(time)
...
function Activity11BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity11BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity11BossBattleWindow.CloseWindow)
end

function Activity11BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity11BossBattleWindow.name)
end

function Activity11BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity11BossBattleWindow
