    ...
    Activity11_MiniGameService.MiniGameInfoReq(function()
      RedDotMgr.AddNode({
        ...
        visibleFunc = function()
          return RedDotActivityDungeon.MiniGame11_TaskRewardable() or RedDotActivityDungeon.MiniGame11_DailyTaskRewardable()
        end,
        dataType = RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON
      })
      RedDotMgr.UpdateNodeByWindowName(WinResConfig.Activity11DungeonWindow.name)
    end)
  end
end

function Activity11DungeonWindow.ReInitData()
end

function Activity11DungeonWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity11DungeonWindow.package, WinResConfig.Activity11DungeonWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1011_ActivityDungeonWindowUis(contentPane)
    ...
    if plotShowId then
      ...
      if reviewWord then
        reviewWord.onClick:Set(function()
          ...
          if id then
            OpenWindow(WinResConfig.Activity11PlotWindow.name, nil, id, configData)
          end
        end)
...
function Activity11DungeonWindow.LoadBg()
  ...
  local bg1 = LuaUtil.FindChild(effect, "eventcovers_0011_1", true)
  local bg2 = LuaUtil.FindChild(effect, "eventcovers_0011_2", true)
  local spineTb = {bg1, bg2}
  for i = 1, #spineTb do
...
function Activity11DungeonWindow.InitRedDot()
  RedDotMgr.AddNode({
    windowName = WinResConfig.Activity11DungeonWindow.name,
...
function Activity11DungeonWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
    ...
    if configData.boss_chapter_ids then
      ...
      if 2 == #arr then
        ...
        uis.Main.BossBtn.onClick:Set(function()
          if lock then
            OpenWindow(WinResConfig.Activity11BossBattleWindow.name)
          else
            FloatTipsUtil.ShowWarnTips(tips)
          end
        end)
...
function Activity11DungeonWindow.InitBtnTxt()
  UIUtil.SetText(uis.Main.BossBtn, T(1914))
  UIUtil.SetText(uis.Main.MaterialBtn, T(1915))
  uis.Main.NormalBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity11ChallengeWindow.name)
  end)
  UIUtil.SetText(uis.Main.NormalBtn, T(1916))
    ...
    if passData then
      uis.Main.PassBtn.onClick:Set(function()
        PassportService.GetBattlePassInfoReq(function()
          ...
          OpenWindow(WinResConfig.Activity11PassportWindow.name)
        end)
      end)
  ...
  UIUtil.SetText(uis.Main.PlotBtn, T(1911))
  uis.Main.ShopBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity11ShopWindow.name)
  end)
  UIUtil.SetText(uis.Main.ShopBtn, T(1913))
  uis.Main.SignBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity11SignWindow.name)
  end)
  UIUtil.SetText(uis.Main.SignBtn, T(1917))
  uis.Main.TaskBtn.onClick:Set(function()
    ActivityDungeonService.GetActivityAllReq(function()
      OpenWindow(WinResConfig.Activity11TaskWindow.name)
    end)
  end)
  UIUtil.SetText(uis.Main.TaskBtn, T(1912))
  uis.Main.MiniGameBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity11MiniGameMainWindow.name)
  end)
end

function Activity11DungeonWindow.InitBtn()
  jumpTb = CurrencyReturnWindow.SetCurrencyReturn(WinResConfig.Activity11DungeonWindow.name, uis.Main.CurrencyReturn, FEATURE_ENUM.HOME_ADVENTURE)
end

function Activity11DungeonWindow.OnShown()
  if uis then
    Activity11DungeonWindow.UpdateInfo()
  end
end

function Activity11DungeonWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity11DungeonWindow.name)
  uis = nil
  if effect then
...
function Activity11DungeonWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_CROSS_DAY then
    ActivityDungeonMgr.CheckActivityEnd()
  end
end

return Activity11DungeonWindow
