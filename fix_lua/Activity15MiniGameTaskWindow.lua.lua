    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity15_MiniGameService.MiniGameInfoReq(RefreshRewardList)
          end
...
function Activity15MiniGameTaskWindow.ReInitData()
end

function Activity15MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity15MiniGameTaskWindow.package, WinResConfig.Activity15MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1015_MiniTaskWindowUis(contentPane)
    Activity15MiniGameTaskWindow.UpdateInfo()
    Activity15MiniGameTaskWindow.InitBtn()
  end)
end

function Activity15MiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity15_MiniGameData.GetMiniGameInfo()
...
function Activity15MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity15MiniGameTaskWindow.name)
  end)
end

function Activity15MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity15MiniGameTaskWindow
