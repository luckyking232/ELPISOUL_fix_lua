...
function Activity12ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity12ChallengeWindow.package, WinResConfig.Activity12ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1012_ChallengeWindowUis(contentPane)
...
function Activity12ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity12ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity12ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity12ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity12ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity12ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity12ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity12ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity12ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity12ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity12ChallengeWindow.ShopInfo()
...
function Activity12ChallengeWindow.GetChapterPassById(id)
...
function Activity12ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity12ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity12ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity12ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity12ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity12ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(1927))
end

function Activity12ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity12ChallengeWindow.name)
end

function Activity12ChallengeWindow.OnShown()
  if uis then
    Activity12ChallengeWindow.UpdateInfo()
  end
end

function Activity12ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity12ChallengeWindow
