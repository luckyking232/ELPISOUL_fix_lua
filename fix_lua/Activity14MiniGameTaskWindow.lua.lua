    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity14_MiniGameService.MiniGameInfoReq(gameId, RefreshRewardList)
          end
...
function Activity14MiniGameTaskWindow.ReInitData()
end

function Activity14MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity14MiniGameTaskWindow.package, WinResConfig.Activity14MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1014_MiniTaskWindowUis(contentPane)
    Activity14MiniGameTaskWindow.UpdateInfo()
    Activity14MiniGameTaskWindow.InitBtn()
  end)
end

function Activity14MiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity14_MiniGameData.GetMiniGameInfo(gameId)
...
function Activity14MiniGameTaskWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity14MiniGameTaskWindow.name)
  end)
...
function Activity14MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity14MiniGameTaskWindow
