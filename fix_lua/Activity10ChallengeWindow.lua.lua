...
function Activity10ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity10ChallengeWindow.package, WinResConfig.Activity10ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1010_ChallengeWindowUis(contentPane)
...
function Activity10ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity10ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity10ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity10ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity10ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity10ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity10ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity10ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity10ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity10ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity10ChallengeWindow.ShopInfo()
...
function Activity10ChallengeWindow.GetChapterPassById(id)
...
function Activity10ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity10ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity10ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity10ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity10ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity10ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(1886))
end

function Activity10ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity10ChallengeWindow.name)
end

function Activity10ChallengeWindow.OnShown()
  if uis then
    Activity10ChallengeWindow.UpdateInfo()
  end
end

function Activity10ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity10ChallengeWindow
