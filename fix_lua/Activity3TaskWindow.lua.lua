...
function Activity3TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity3TaskWindow.package, WinResConfig.Activity3TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1002_TaskWindowUis(contentPane)
...
function Activity3TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity3TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity3TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask() and true or false
  end
end

function Activity3TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity3TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity3TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity3TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity3TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(1652)
  title.TitleTxt.text = T(1651)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity3TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity3TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity3TaskWindow.name)
end

function Activity3TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity3TaskWindow
