...
function ActivityCasket3Window.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.ActivityCasket3Window.package, WinResConfig.ActivityCasket3Window.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityCasket02_CasketWindowUis(contentPane)
...
function ActivityCasket3Window.GetPoolInfo(pools, id)
...
function ActivityCasket3Window.UpdateInfo(poolId)
  ...
  if turnTableConfig then
    if nil == mapCom then
      mapCom = UIMgr:CreateObject("ActivityCasket02", "TagRegion_01")
      uis.Main.TagRegion.root:AddChild(mapCom)
    end
    allData = ActivityCasket3Window.GetData(turnTableConfig.group_id)
    maxNum = #allData
    for i = 1, maxNum do
      local tips = mapCom:GetChild("Tag" .. i)
      local arr = GetConfigItemList(allData[i].reward, true)
      local poolInfo = ActivityCasket3Window.GetPoolInfo(turnTableInfo.pools, allData[i].id)
      if tips and arr[1] then
        if poolId and poolInfo and poolInfo.poolId == poolId then
          ...
          if arr[1].type == ProtoEnum.TUPLE_TYPE.T_FASHION then
            ...
            UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/turntable/FX_ui_turntable_zhounian2_midlight_open_get.prefab", tempHolder, nil, true)
          else
        ...
        if arr[1].type == ProtoEnum.TUPLE_TYPE.T_FASHION then
          ...
          if nil == boxObj then
            boxObj = UIUtil.SetEffectToUI("Assets/Art/Models/UI_spine/prefab/ui_Turntable_skin_gift_mh.prefab", tips:GetChild("BoxHolder"))
          end
  ...
  if maxNum and turnTableInfo and turnTableInfo.turnNum and turnTableInfo.turnNum >= maxNum then
    ...
    local cost = ActivityCasket3Window.GetCost()
    ...
    uis.Main.GaChaBtn.onClick:Set(function()
      ...
      if canDo then
        ...
        SignService.ActivityDoTurnTableReq(turnTableInfo.baseInfo.activityId, function(msg)
          ...
          animPlay = true
          ActivityCasket3Window.PlanAnim(tempPools, msg.poolId, function()
            if uis then
              uis.Main.GaChaBtn.touchable = true
              ActivityCasket3Window.UpdateInfo(msg.poolId)
...
function ActivityCasket3Window.PlanAnim(tempPools, poolId, callback)
  curIndex = curIndex + 1
  local indexTb = {}
  local tempInd
  for i = 1, #allData do
    local poolInfo = ActivityCasket3Window.GetPoolInfo(tempPools, allData[i].id)
  ...
  for i = 1, len do
    ...
    TimerUtil.setTimeout(i * time, function()
      if uis then
        ...
        if 10 == newTb[i] then
          SoundUtil.PlayUISfx("event:/sfx/sfx_ui/ui_sys/ui_sfx_act9_rotary_reward_box")
          UIUtil.SetEffectToUI("Assets/Art/Effects/Prefab/UI_prefab/turntable/FX_ui_turntable2_midlight_open.prefab", tips:GetChild("EffectHolder"), nil, true)
        else
        ...
        if i == len then
          if curIndex <= tatalNum then
            LeanTween.delayedCall(time, function()
              ActivityCasket3Window.PlanAnim(tempPools, poolId, callback)
            end)
...
function ActivityCasket3Window.GetData(groupId)
...
function ActivityCasket3Window.GetCost()
...
function ActivityCasket3Window.InitAssetsTips()
  ...
  function uis.Main.AssetsTipsList.itemRenderer(i, item)
    ...
    item:GetChild("AssetsBtn").onClick:Set(function()
      uis.Main.AssetsTipsList.onClick:Set(function()
        OpenWindow(WinResConfig.ActivityCasket3ExplainTipsWindow.name, nil, 0, turnTableConfig, turnTableInfo.baseInfo.activityId)
      end)
    end)
...
function ActivityCasket3Window.InitBtn()
  local btnTb = {
    [1] = {
  ...
  for i = 1, #btnTb do
    ...
    btnTb[i].btn.onClick:Set(function()
      OpenWindow(WinResConfig.ActivityCasket3ExplainTipsWindow.name, nil, btnTb[i].ctr, turnTableConfig, turnTableInfo.baseInfo.activityId)
    end)
  end
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.ActivityCasket3Window.name)
  end)
  UIUtil.SetText(uis.Main.ShopBtn, T(1877), "TagTxt")
end

function ActivityCasket3Window.OnShown()
  if uis then
    ActivityCasket3Window.UpdateInfo()
    showBoxOpen = nil
  end
...
function ActivityCasket3Window.OnClose()
  uis = nil
  contentPane = nil
  turnTableConfig = nil
  turnTableInfo = nil
...
function ActivityCasket3Window.CheckActivityEnd()
...
function ActivityCasket3Window.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_CROSS_DAY then
    ActivityCasket3Window.CheckActivityEnd()
  end
end

return ActivityCasket3Window
