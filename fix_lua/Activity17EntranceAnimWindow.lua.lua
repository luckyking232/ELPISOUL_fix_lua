...
local PATH_LOOKUP = {
  [1] = "Assets/Art/Models/UI_spine/prefab/Eventcovers/eventcovers_0017.prefab"
}
local currentNum, currentEff
...
function Activity17EntranceAnimWindow.ReInitData()
end

function Activity17EntranceAnimWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity17EntranceAnimWindow.package, WinResConfig.Activity17EntranceAnimWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1017_MainSeasonsWindowUis(contentPane)
    Activity17EntranceAnimWindow.UpdateInfo()
    Activity17EntranceAnimWindow.InitBtn()
  end)
end

function Activity17EntranceAnimWindow.UpdateInfo()
  PlayDailyEntranceAnim(1, 1)
end

function Activity17EntranceAnimWindow.InitBtn()
  uis.Main.root.onClick:Set(function()
    if touchable then
      ...
      tweenId = LeanTween.value(1, 0, 0.5):setOnUpdate(function(val)
        ...
        RemoveTween(tweenId)
        holder:Dispose()
        UIMgr:CloseToWindow(WinResConfig.Activity17DungeonWindow.name)
      end).id
      table.insert(tweeners, tweenId)
      touchable = false
    end
...
function Activity17EntranceAnimWindow.OnClose()
  uis = nil
  contentPane = nil
  currentNum = nil
  currentEff = nil
...
return Activity17EntranceAnimWindow
