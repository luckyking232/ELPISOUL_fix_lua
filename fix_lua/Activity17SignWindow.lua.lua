...
function Activity17SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity17SignWindow.package, WinResConfig.Activity17SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1017_SignWindowUis(contentPane)
    Activity17SignWindow.UpdateInfo()
    Activity17SignWindow.InitBtn()
  end)
end

function Activity17SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity17SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity17SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity17SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity17SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity17SignWindow.GetRewardData(groupId)
...
function Activity17SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity17SignWindow.CloseWindow)
end

function Activity17SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity17SignWindow.name)
end

function Activity17SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity17SignWindow
