...
function Activity6ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity6ShopWindow.package, WinResConfig.Activity6ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1005_ShopWindowUis(contentPane)
    Activity6ShopWindow.UpdateInfo()
    Activity6ShopWindow.InitBtn()
  end)
end

function Activity6ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity6ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity6ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity6ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity6ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity6ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity6ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity6ShopWindow.GetShopGrid(groupId)
...
function Activity6ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity6ShopWindow.CloseWindow)
end

function Activity6ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity6ShopWindow.name)
end

function Activity6ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity6ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity6ShopWindow.UpdateInfo(true)
  end
end

return Activity6ShopWindow
