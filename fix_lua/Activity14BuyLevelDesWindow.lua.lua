...
function Activity14BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity14BuyLevelDesWindow.package, WinResConfig.Activity14BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1014_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity14BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity14BuyLevelDesWindow.ShowAssetsTips()
...
function Activity14BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity14BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity14BuyLevelDesWindow.GetGold(buyNum)
...
function Activity14BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity14BuyLevelDesWindow.ShowReward()
  local data = Activity14BuyLevelDesWindow.GetRewardItem()
...
function Activity14BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity14BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity14BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity14BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity14BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity14BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity14PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity14BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity14PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity14BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity14BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity14BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity14BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity14BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity14BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity14BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity14BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity14BuyLevelDesWindow
