...
function Activity7ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity7ShopWindow.package, WinResConfig.Activity7ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1006_ShopWindowUis(contentPane)
    Activity7ShopWindow.UpdateInfo()
    Activity7ShopWindow.InitBtn()
  end)
end

function Activity7ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity7ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity7ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity7ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity7ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity7ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity7ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity7ShopWindow.GetShopGrid(groupId)
...
function Activity7ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity7ShopWindow.CloseWindow)
end

function Activity7ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity7ShopWindow.name)
end

function Activity7ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity7ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity7ShopWindow.UpdateInfo(true)
  end
end

return Activity7ShopWindow
