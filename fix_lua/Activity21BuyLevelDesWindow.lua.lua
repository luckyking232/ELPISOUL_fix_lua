...
function Activity21BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity21BuyLevelDesWindow.package, WinResConfig.Activity21BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1021_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity21BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity21BuyLevelDesWindow.ShowAssetsTips()
...
function Activity21BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity21BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity21BuyLevelDesWindow.GetGold(buyNum)
...
function Activity21BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity21BuyLevelDesWindow.ShowReward()
  local data = Activity21BuyLevelDesWindow.GetRewardItem()
...
function Activity21BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity21BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity21BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity21BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity21BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity21BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity21PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity21BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity21PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity21BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity21BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity21BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity21BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity21BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity21BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity21BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity21BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity21BuyLevelDesWindow
