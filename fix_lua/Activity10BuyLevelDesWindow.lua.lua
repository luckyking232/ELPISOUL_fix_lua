...
function Activity10BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity10BuyLevelDesWindow.package, WinResConfig.Activity10BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1010_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity10BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity10BuyLevelDesWindow.ShowAssetsTips()
...
function Activity10BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity10BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity10BuyLevelDesWindow.GetGold(buyNum)
...
function Activity10BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity10BuyLevelDesWindow.ShowReward()
  local data = Activity10BuyLevelDesWindow.GetRewardItem()
...
function Activity10BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity10BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity10BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity10BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity10BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity10BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity10PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity10BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity10PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity10BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity10BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity10BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity10BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity10BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity10BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity10BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity10BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity10BuyLevelDesWindow
