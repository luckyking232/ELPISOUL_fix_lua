...
function Activity4BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity4BossBattleWindow.package, WinResConfig.Activity4BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1003_BossBattleWindowUis(contentPane)
    Activity4BossBattleWindow.UpdateInfo()
    Activity4BossBattleWindow.InitBtn()
  end)
end

function Activity4BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity4BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity4BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity4BossBattleWindow.FormatRemainTime(time)
...
function Activity4BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity4BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity4BossBattleWindow.CloseWindow)
end

function Activity4BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity4BossBattleWindow.name)
end

function Activity4BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity4BossBattleWindow
