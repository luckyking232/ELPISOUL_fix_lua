...
function Activity6ChallengeWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity6ChallengeWindow.package, WinResConfig.Activity6ChallengeWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1005_ChallengeWindowUis(contentPane)
...
function Activity6ChallengeWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  if activityInfo then
    local firstChapterData = Activity6ChallengeWindow.GetFirstChapter()
    local curChapterData, curStage = Activity6ChallengeWindow.GetCurStage(firstChapterData)
    if curChapterData and curStage then
      AdventureData.SaveDailyStageNew(curChapterData.id, true)
      local chapterIds = Activity6ChallengeWindow.GetAllChapterId()
      ...
      function uis.Main.ChapterList.itemRenderer(i, item)
        ...
        if tempData then
          ...
          local lock = Activity6ChallengeWindow.GetChapterPassById(tempData.pre)
          if curTime >= activityInfo.normalChapter[tempData.id] and lock then
            ...
            item.onClick:Set(function()
              ...
              Activity6ChallengeWindow.ShoweStage(map, tempData, true)
            end)
          else
            local tipsTxt
...
function Activity6ChallengeWindow.ShoweStage(map, chapterData, anim)
  local curChapterData, curStage, pass = Activity6ChallengeWindow.GetCurStage(chapterData, true)
  local x = 0
  for i = 1, #chapterData.stages do
    ...
    if stageBtn and stageData then
      ...
      elseif stageData.id == curStage then
        ...
        stageBtn.onClick:Set(function()
          if not pass then
            Activity6ChallengeWindow.StageItemClick(stageData)
          else
            FloatTipsUtil.ShowWarnTips(T(1540))
          end
        end)
...
function Activity6ChallengeWindow.StageItemClick(stageData, resetStageFunc)
...
function Activity6ChallengeWindow.ShopInfo()
...
function Activity6ChallengeWindow.GetChapterPassById(id)
...
function Activity6ChallengeWindow.GetCurStage(chapterData, isChapter)
  if chapterData then
    local stage = activityInfo.finishStages
    local endStage = chapterData.stages[#chapterData.stages]
    if table.contain(stage, endStage) then
      if chapterData.next and chapterData.next > 0 and nil == isChapter then
        ...
        if nextData and LoginData.GetCurServerTime() > activityInfo.normalChapter[nextData.id] then
          return Activity6ChallengeWindow.GetCurStage(nextData)
        else
...
function Activity6ChallengeWindow.GetFirstChapter()
  local chapterData
...
function Activity6ChallengeWindow.GetAllChapterId()
  local chapterId = {}
  for i, v in pairs(activityInfo.normalChapter) do
...
function Activity6ChallengeWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity6ChallengeWindow.CloseWindow)
  UIUtil.SetText(uis.Main.ShopBtn, T(1764))
end

function Activity6ChallengeWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity6ChallengeWindow.name)
end

function Activity6ChallengeWindow.OnShown()
  if uis then
    Activity6ChallengeWindow.UpdateInfo()
  end
end

function Activity6ChallengeWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
  if scrollingTween then
...
return Activity6ChallengeWindow
