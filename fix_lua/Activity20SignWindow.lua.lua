...
function Activity20SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity20SignWindow.package, WinResConfig.Activity20SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1020_SignWindowUis(contentPane)
    Activity20SignWindow.UpdateInfo()
    Activity20SignWindow.InitBtn()
  end)
end

function Activity20SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity20SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity20SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity20SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity20SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity20SignWindow.GetRewardData(groupId)
...
function Activity20SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity20SignWindow.CloseWindow)
end

function Activity20SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity20SignWindow.name)
end

function Activity20SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity20SignWindow
