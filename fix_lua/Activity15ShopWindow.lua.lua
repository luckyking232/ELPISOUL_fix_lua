...
function Activity15ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity15ShopWindow.package, WinResConfig.Activity15ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1015_ShopWindowUis(contentPane)
    Activity15ShopWindow.UpdateInfo()
    Activity15ShopWindow.InitBtn()
  end)
end

function Activity15ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity15ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity15ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity15ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity15ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity15ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity15ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity15ShopWindow.GetShopGrid(groupId)
...
function Activity15ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity15ShopWindow.CloseWindow)
end

function Activity15ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity15ShopWindow.name)
end

function Activity15ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity15ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity15ShopWindow.UpdateInfo(true)
  end
end

return Activity15ShopWindow
