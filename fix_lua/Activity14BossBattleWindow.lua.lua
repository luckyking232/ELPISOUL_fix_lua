...
function Activity14BossBattleWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity14BossBattleWindow.package, WinResConfig.Activity14BossBattleWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1014_BossBattleWindowUis(contentPane)
    Activity14BossBattleWindow.UpdateInfo()
    Activity14BossBattleWindow.InitBtn()
  end)
end

function Activity14BossBattleWindow.UpdateInfo()
  ...
  if activityInfo then
    ...
    local chapterId = Activity14BossBattleWindow.GetChapter()
    if chapterId then
            ...
            if stageData.unlock_time == nil or stageData.unlock_time then
              if stageData.unlock_time == nil or curTime >= activityInfo.baseInfo.startStamp + stageData.unlock_time then
                ...
                if stageData.unlock_time then
                  ...
                  UIUtil.SetText(tips, lockShow and Activity14BossBattleWindow.FormatRemainTime(time) or "", "LockTxt")
                end
                ChangeUIController(tips, "c1", 0)
                lockShow = nil
              end
...
function Activity14BossBattleWindow.FormatRemainTime(time)
...
function Activity14BossBattleWindow.GetChapter()
  for i, v in pairs(activityInfo.bossChapter) do
    return i
  end
end

function Activity14BossBattleWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity14BossBattleWindow.CloseWindow)
end

function Activity14BossBattleWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity14BossBattleWindow.name)
end

function Activity14BossBattleWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity14BossBattleWindow
