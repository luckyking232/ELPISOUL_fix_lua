...
function Activity5MaterialWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity5MaterialWindow.package, WinResConfig.Activity5MaterialWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1004_MaterialWindowUis(contentPane)
...
function Activity5MaterialWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
  maxCount = Activity5MaterialWindow.GetMaxCount()
  if activityInfo then
    local id = Activity5MaterialWindow.GetChapter()
    if id then
...
function Activity5MaterialWindow.GetChapter()
  for i, v in pairs(activityInfo.creamChapter) do
    return i
  end
end

function Activity5MaterialWindow.GetMaxCount()
...
function Activity5MaterialWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity5MaterialWindow.CloseWindow)
end

function Activity5MaterialWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity5MaterialWindow.name)
end

function Activity5MaterialWindow.OnShown()
  if uis then
    Activity5MaterialWindow.UpdateInfo()
  end
end

function Activity5MaterialWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity5MaterialWindow
