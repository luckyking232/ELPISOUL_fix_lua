...
local InitMiniGameInfo = function()
  ...
  if data.id == 70440001 then
    ...
    Activity1_MiniGameService.MiniGameInfoReq(function()
      ...
      RedDotMgr.UpdateNodeByWindowName(WinResConfig.Activity4DungeonWindow.name)
    end)
...
function Activity4DungeonWindow.ReInitData()
end

function Activity4DungeonWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity4DungeonWindow.package, WinResConfig.Activity4DungeonWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1003_ActivityDungeonWindowUis(contentPane)
    ...
    if plotShowId then
      ...
      if reviewWord then
        reviewWord.onClick:Set(function()
          ...
          if id then
            OpenWindow(WinResConfig.Activity4PlotWindow.name, nil, id, configData)
          end
        end)
...
function Activity4DungeonWindow.LoadBg()
  ...
  local spine = LuaUtil.FindChild(effect, "eventcovers_0004", true)
...
function Activity4DungeonWindow.InitRedDot()
  RedDotMgr.AddNode({
    windowName = WinResConfig.Activity4DungeonWindow.name,
...
function Activity4DungeonWindow.UpdateInfo()
  activityInfo = ActivityDungeonData.GetActivityInfo()
    ...
    if configData.boss_chapter_ids then
      ...
      if 2 == #arr then
        ...
        uis.Main.BossBtn.onClick:Set(function()
          if lock then
            OpenWindow(WinResConfig.Activity4BossBattleWindow.name)
          else
            FloatTipsUtil.ShowWarnTips(tips)
          end
        end)
...
function Activity4DungeonWindow.InitBtnTxt()
  UIUtil.SetText(uis.Main.BossBtn, T(1669))
  UIUtil.SetText(uis.Main.MaterialBtn, T(1670))
  uis.Main.NormalBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity4ChallengeWindow.name)
  end)
  UIUtil.SetText(uis.Main.NormalBtn, T(1671))
    ...
    if passData then
      uis.Main.PassBtn.onClick:Set(function()
        PassportService.GetBattlePassInfoReq(function()
          ...
          OpenWindow(WinResConfig.Activity4PassportWindow.name)
        end)
      end)
  ...
  UIUtil.SetText(uis.Main.PlotBtn, T(1666))
  uis.Main.ShopBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity4ShopWindow.name)
  end)
  UIUtil.SetText(uis.Main.ShopBtn, T(1668))
  uis.Main.SignBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity4SignWindow.name)
  end)
  UIUtil.SetText(uis.Main.SignBtn, T(1672))
  uis.Main.TaskBtn.onClick:Set(function()
    ActivityDungeonService.GetActivityAllReq(function()
      OpenWindow(WinResConfig.Activity4TaskWindow.name)
    end)
  end)
  UIUtil.SetText(uis.Main.TaskBtn, T(1667))
  uis.Main.MiniGameBtn.onClick:Set(function()
    OpenWindow(WinResConfig.Activity4MiniGameMainWindow.name)
  end)
end

function Activity4DungeonWindow.InitBtn()
  jumpTb = CurrencyReturnWindow.SetCurrencyReturn(WinResConfig.Activity4DungeonWindow.name, uis.Main.CurrencyReturn, FEATURE_ENUM.HOME_ADVENTURE)
end

function Activity4DungeonWindow.OnShown()
  if uis then
    Activity4DungeonWindow.UpdateInfo()
  end
end

function Activity4DungeonWindow.OnClose()
  RedDotMgr.RemoveNode(WinResConfig.Activity4DungeonWindow.name)
  uis = nil
  if effect then
...
function Activity4DungeonWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_CROSS_DAY then
    ActivityDungeonMgr.CheckActivityEnd()
  end
end

return Activity4DungeonWindow
