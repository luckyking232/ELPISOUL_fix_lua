...
function Activity9BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity9BuyLevelDesWindow.package, WinResConfig.Activity9BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1008_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity9BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity9BuyLevelDesWindow.ShowAssetsTips()
...
function Activity9BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity9BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity9BuyLevelDesWindow.GetGold(buyNum)
...
function Activity9BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity9BuyLevelDesWindow.ShowReward()
  local data = Activity9BuyLevelDesWindow.GetRewardItem()
...
function Activity9BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity9BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity9BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity9BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity9BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity9BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity9PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity9BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity9PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity9BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity9BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity9BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity9BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity9BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity9BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity9BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity9BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity9BuyLevelDesWindow
