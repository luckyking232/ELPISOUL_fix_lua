    ...
    if state == ProtoEnum.TASK_STATE.FINISHED then
      ...
      child:GetChild("GetBtn").onClick:Set(function()
        ...
        ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, task.uid, function()
          if uis then
            Activity9_MiniGameService.MiniGameInfoReq(Activity9_MiniGameData.gameId, RefreshRewardList)
          end
...
function Activity9MiniGameTaskWindow.ReInitData()
end

function Activity9MiniGameTaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity9MiniGameTaskWindow.package, WinResConfig.Activity9MiniGameTaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1008_MiniTaskWindowUis(contentPane)
    Activity9MiniGameTaskWindow.UpdateInfo()
    Activity9MiniGameTaskWindow.InitBtn()
  end)
end

function Activity9MiniGameTaskWindow.UpdateInfo()
  RefreshRewardList()
  local info = Activity9_MiniGameData.GetOneMiniGameInfo()
...
function Activity9MiniGameTaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity9MiniGameTaskWindow.name)
  end)
end

function Activity9MiniGameTaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity9MiniGameTaskWindow
