...
function Activity13SignWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity13SignWindow.package, WinResConfig.Activity13SignWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1013_SignWindowUis(contentPane)
    Activity13SignWindow.UpdateInfo()
    Activity13SignWindow.InitBtn()
  end)
end

function Activity13SignWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local signData = Activity13SignWindow.GetRewardData(data.check_in_id)
      local toIndex
      ...
      function list.RewardList.itemRenderer(i, item)
          ...
          if table.contain(activityInfo.signDay, signData[i + 1].day) then
            ...
            rewardItem.onClick:Set(function()
              ActivityDungeonService.ActivityStageSignInReq(signData[i + 1].day, activityInfo.baseInfo.activityId, function()
                if uis then
                  Activity13SignWindow.UpdateInfo(true)
                end
              end)
            end)
            toIndex = i
        ...
        if arr[1] then
          Activity13SignWindow.ShowAllItemFrame(arr[1], rewardItem:GetChild("Item"), function()
            if showTips then
...
function Activity13SignWindow.ShowAllItemFrame(addAttr, item, callBack)
...
function Activity13SignWindow.GetRewardData(groupId)
...
function Activity13SignWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity13SignWindow.CloseWindow)
end

function Activity13SignWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity13SignWindow.name)
end

function Activity13SignWindow.OnClose()
  uis = nil
  contentPane = nil
  activityInfo = nil
end

return Activity13SignWindow
