...
function Activity3DungeonWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity3DungeonWindow.package, WinResConfig.Activity3DungeonWindow.comName, function(com)
    ...
    local recallBol = bridgeObj.argTable[1]
    ChangeUIController(uis.Main.root, "review", recallBol and 1 or 0)
    ...
    if plotShowId then
      ...
      if reviewWord then
        reviewWord.onClick:Set(function()
          ...
          if id then
            OpenWindow(WinResConfig.Activity3PlotWindow.name, nil, id, configData)
          end
        end)
...
function Activity3DungeonWindow.LoadBg()
