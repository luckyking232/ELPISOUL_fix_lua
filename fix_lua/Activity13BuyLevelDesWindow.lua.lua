...
function Activity13BuyLevelDesWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity13BuyLevelDesWindow.package, WinResConfig.Activity13BuyLevelDesWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1013_BuyLevelDesWindowUis(contentPane)
    ...
    if passInfo then
      ...
      Activity13BuyLevelDesWindow.GetLvData(passportData.grow_model_id, curLv)
...
function Activity13BuyLevelDesWindow.ShowAssetsTips()
...
function Activity13BuyLevelDesWindow.UpdateTextDisplay()
...
function Activity13BuyLevelDesWindow.Init()
  if lvData then
    if lvData[1] then
    ...
    local UpdateBuyNum = function()
      ...
      local costGold = Activity13BuyLevelDesWindow.GetGold(buyNum)
...
function Activity13BuyLevelDesWindow.GetGold(lv)
  local gold = 0
  for i = 1, lv do
    if lvData[i] then
...
function Activity13BuyLevelDesWindow.ShowReward()
  local data = Activity13BuyLevelDesWindow.GetRewardItem()
...
function Activity13BuyLevelDesWindow.GetRewardItem()
  local reward
  local item = {}
...
function Activity13BuyLevelDesWindow.GetLvData(type, lv)
  lvData = {}
...
function Activity13BuyLevelDesWindow.GetRewardLvData()
  ...
  if passPortConfig then
    for i = 1, #passPortConfig.pass_ids do
      if table.contain(passInfo.openList, passPortConfig.pass_ids[i]) then
        rewardData[i] = Activity13BuyLevelDesWindow.GetRewardByPhaseId(passPortConfig.pass_ids[i])
      end
    end
  end
end

function Activity13BuyLevelDesWindow.GetRewardByPhaseId(phaseId)
  local data = {}
...
function Activity13BuyLevelDesWindow.BuyLevelRsp(msg)
  if msg.level then
    passInfo.lv = msg.level
    UIMgr:SendWindowMessage(WinResConfig.Activity13PassportWindow.name, WindowMsgEnum.Passport.BUY, msg.level)
    UIMgr:CloseWindow(WinResConfig.Activity13BuyLevelDesWindow.name)
    OpenWindow(WinResConfig.Activity13PassportLevelUpTipsWindow.name, nil, msg.level)
  end
  RedDotMgr.UpdateNode(RED_DOT_DATA_TYPE.ACTIVITY_DUNGENON)
end

function Activity13BuyLevelDesWindow.InitBtn()
  uis.Main.TouchScreenBtn.onClick:Set(function()
    UIMgr:CloseWindow(WinResConfig.Activity13BuyLevelDesWindow.name)
    SoundUtil.PlayUISfx(SOUND_EVENT_ENUM.POPUP_WINDOW_CLOSE)
  end)
  uis.Main.BuyLevelDes1.BuyLevelBtn.onClick:Set(function()
    local tGold = Activity13BuyLevelDesWindow.GetGold(buyNum)
    local gold = ActorData.GetItemCount(itemId)
    if tGold <= gold then
      PassportService.BattlePassBuyLevelReq(passInfo.passPortId, buyNum, function(msg)
        Activity13BuyLevelDesWindow.BuyLevelRsp(msg)
      end)
    else
...
function Activity13BuyLevelDesWindow.HandleMessage(msgId, para)
  if msgId == WindowMsgEnum.Common.E_MSG_ITEM_CHANGE then
    Activity13BuyLevelDesWindow.ShowAssetsTips()
  end
end

function Activity13BuyLevelDesWindow.OnShown()
  if uis and waitFun then
    waitFun()
  end
end

function Activity13BuyLevelDesWindow.OnClose()
  uis = nil
  contentPane = nil
  lvData = nil
  curLv = nil
...
return Activity13BuyLevelDesWindow
