...
function Activity10ShopWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity10ShopWindow.package, WinResConfig.Activity10ShopWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1010_ShopWindowUis(contentPane)
    Activity10ShopWindow.UpdateInfo()
    Activity10ShopWindow.InitBtn()
  end)
end

function Activity10ShopWindow.UpdateInfo(noPlayAnim)
  ...
  if activityInfo then
    ...
    if data then
      ...
      local shopGrid = Activity10ShopWindow.GetShopGrid(data.shop_id)
      local titleTxt = {
        T(1529),
        T(1530),
        T(1617)
      ...
      function uis.Main.ItemList.itemRenderer(i, item)
        ...
        Activity10ShopWindow.ShowItem(list, shopGrid[i + 1], noPlayAnim)
      end
...
function Activity10ShopWindow.ShowItem(list, shopGrid, noPlayAnim)
  function list.itemRenderer(i, item)
    local data = shopGrid[i + 1]
    
    local tokenCom = item:GetChild("Item")
    local itemBtn = tokenCom:GetChild("ItemBtn")
    local itemData, ctrType = Activity10ShopWindow.ShowOneReward(data.item[1], itemBtn, "ItemLoader", "NumberTxt", true)
    if itemData then
    ...
    Activity10ShopWindow.ShowOneReward(data.sell_price[1], itemBtn, "SpendLoader", "Spend")
...
function Activity10ShopWindow.ShowOneReward(reward, item, loaderName, numberTxtName, showStr, smallIcon)
...
function Activity10ShopWindow.GetShopGrid(groupId)
...
function Activity10ShopWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity10ShopWindow.CloseWindow)
end

function Activity10ShopWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity10ShopWindow.name)
end

function Activity10ShopWindow.OnClose()
  uis = nil
  contentPane = nil
  tempAssetsTips = nil
  activityInfo = nil
...
function Activity10ShopWindow.HandleMessage(msgId, para)
  if WindowMsgEnum.ActivityDungeon.REFRESH_SHOP == msgId then
    Activity10ShopWindow.UpdateInfo(true)
  end
end

return Activity10ShopWindow
