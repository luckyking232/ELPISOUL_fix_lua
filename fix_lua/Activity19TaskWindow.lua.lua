...
function Activity19TaskWindow.OnInit(bridgeObj)
  bridgeObj:SetViewAsync(WinResConfig.Activity19TaskWindow.package, WinResConfig.Activity19TaskWindow.comName, function(com)
    contentPane = com
    contentPane:Center()
    uis = GetActivityDungeon1019_TaskWindowUis(contentPane)
...
function Activity19TaskWindow.UpdateInfo(notPlayAnim)
  ...
  if activityInfo then
    ...
    function list.itemRenderer(i, item)
      ...
      if taskData then
        ...
        function rewardList.itemRenderer(i, item)
          Activity19TaskWindow.ShowAllItemFrame(arr[1 + i], item)
        end
    ...
    title.NumberTxt.text = T(1641, Activity19TaskWindow.GetRewardNum(), #taskList)
    uis.Main.AllGetBtn.visible = RedDotActivityDungeon.CanTask(ActivityDungeonData.GetShowId()) and true or false
  end
end

function Activity19TaskWindow.GetRewardNum()
  local activityInfo = ActivityDungeonData.GetActivityInfo()
  local num = 0
  if activityInfo then
...
function Activity19TaskWindow.ShowAllItemFrame(addAttr, item)
...
function Activity19TaskWindow.SortTask(taskList)
  local list = {}
...
function Activity19TaskWindow.InitBtn()
  uis.Main.ReturnBtn.onClick:Set(Activity19TaskWindow.CloseWindow)
  local title = uis.Main.TaskTitle
  title.WordTxt.text = T(2131)
  title.TitleTxt.text = T(2130)
  ...
  uis.Main.AllGetBtn.onClick:Set(function()
    ...
    if activityInfo then
      ActivityDungeonService.ActivityStageTaskRewardReq(activityInfo.baseInfo.activityId, 0, function()
        if uis then
          Activity19TaskWindow.UpdateInfo(true)
        end
      end)
    end
  end)
...
function Activity19TaskWindow.CloseWindow()
  UIMgr:CloseWindow(WinResConfig.Activity19TaskWindow.name)
end

function Activity19TaskWindow.OnClose()
  uis = nil
  contentPane = nil
end

return Activity19TaskWindow
