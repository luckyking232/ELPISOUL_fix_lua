...
function BattleBuff.NewBuff(_id, _from, _to, _settleNow, _extraParams, _delayParams)
  ...
  function buff:Settle(canSettle, triggerReset, param)
    ...
    if settleUnit then
      ...
      elseif true == self:ContainEffectId(BUFF_EFFECT_ID.SUMMON_FEI) then
        local summonId = BattleBuffMgr.GetValueById(settleUnit, BUFF_EFFECT_ID.SUMMON_FEI)
        local summonParam = {
          summonId = summonId,
          bindUnitSlot = "Flame" .. #settleUnit.cachedSummonUids + 1,
          camp = settleUnit.camp,
          state = BATTLE_STATE_ENUM.START,
          summonFromUnit = settleUnit.uid
        }
  ...
  function buff:Deduce(param)
    ...
    if settleUnit then
      ...
      elseif self.deduceType == BUFF_DEDUCE_TYPE.SETTLE_EFFECT_1052_VALUE then
        local needValue = self.buffConfig.deduce_value
        local effectCount = settleUnit.curValue_1052
        if needValue > effectCount then
          return
        end
    ...
    if settleUnit then
      ...
      if self:ContainEffectId(BUFF_EFFECT_ID.REMOVE_ALL_SUMMON) then
        settleUnit:RemoveAllSummon()
      end
      if self:ContainEffectId(BUFF_EFFECT_ID.SPECIAL_1054) then
        settleUnit:ClearValue_1052()
      end
    end
  ...
  function buff:DealActiveForeverEffect(settleUnit, settleBullet, settleBomb, param)
    ...
    if unit and 0 ~= hpValue then
      ...
      if unit:CanTargetChosen() == false and false == unit:IsRevive() then
        ...
        if 1 ~= self.buffConfig.not_included_in_statistics then
          unit:SaveDamaged(realNum + absorbHurt, fromUnit and fromUnit.uid ~= unit.uid, nil, fromUnit and fromUnit.camp == unit.enemyCamp)
        end
        if fromUnit then
